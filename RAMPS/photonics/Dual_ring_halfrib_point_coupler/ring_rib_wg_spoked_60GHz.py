...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def draw_layout(self):
        ...
        self.draw_one_unit()
        # self.put_gratings()
        self.rout_to_gratings()
        
    def rout_to_gratings(self):
        x_offset_L = 775
        x_offset_R = 125
        y_offset_L = 370
        y_offset_R = 370
        x_pitch_L = 20
        y_pitch = 7.709-0.266+21.031
        
        Wg_L = AdiabaticRouter(gen_cls=self, init_port=self.get_photonic_port('PORT_IN'),
                             layer=('si_full_free', 'drawing'), name='init_port')

        Wg_L.add_straight_wg(length=20, width= 0.35)
        Wg_L.add_straight_wg(length=40, width= 0.35)
        Wg_L.add_offset_bend(offset=y_offset_L+y_pitch-27.886,rmin=10, width= 0.35)
        Wg_L.add_straight_wg(length=x_offset_L-20-40-10, width= 0.35)
...
#+self.coup_gap+self.r_core_cent+self.params['coup_core_width']/2+self.core_width/2
    ...
    def place_contact_electrodes_basic(self):
        ...
        wire1_top = 1+ self.modu_centre[1]
        wire1_left = self.modu_centre[0] - (self.r_r_gap / 2) - self.r_core_cent + 3
        wire1_right = self.modu_centre[0] + (self.r_r_gap / 2) + self.r_core_cent - 3
        self.add_rect(layer=self.inner_electrode_ring_layers[-1],
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
