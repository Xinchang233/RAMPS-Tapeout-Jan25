...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def draw_input_wg_cena(self):
        ...
        master_peanut.move_by(dx=trans_vec_peanut[0] - 2 * self.r_core_cent - self.core_width - self.params['coup_core_width'] / 2 - self.coup_gap - self.r_r_gap / 2,dy=trans_vec_peanut[1],unit_mode=False)
    ...
    #
    #     Wg.add_straight_wg(length=10, width=0.35)
    #
    #     self.extract_photonic_ports(
    ...
    #                           layer=('si_full_free', 'drawing'), name='init_port')
    #
    #     Wg1.add_straight_wg(length=10, width=0.35)
    #
    #     self.extract_photonic_ports(
    ...
    def draw_input_wg(self):
        ...
        if self.bent_coupler:
            ...
            #       self.ring_loc[1] - max(core_master.arc_list[3]['y']) -
            #       self.r_core_cent - self.coup_gap - (self.core_width + self.coup_core_width) / 2)
            loc = (self.ring_loc[0] - max(core_master.arc_list[3]['y']) - (self.r_r_gap / 2 + self.core_width ) -self.coup_core_width/2- self.coup_gap- 2*self.r_core_cent,
                   self.ring_loc[1] - core_master.arc_list[-1]['x'][-1] / 2 )
    ...
    def place_contact_electrodes(self):
        ...
        wire1_top = -3.5
        wire1_bottom = -4
        wire1_left = self.ring_loc[0] - (self.r_r_gap / 2) - self.r_core_cent - 0.3+1.5
        wire1_right = self.ring_loc[0] + (self.r_r_gap / 2) + self.r_core_cent + 0.3-1.5
        ...
        wire1_top = -3
        wire1_bottom = -3.5
        wire1_left = self.ring_loc[0] - (self.r_r_gap / 2) - self.r_core_cent + 1+1.8
        wire1_right = self.ring_loc[0] + (self.r_r_gap / 2) + self.r_core_cent - 1-1.8
        self.add_rect(layer=self.inner_electrode_ring_layers[-1],
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
        ...
        wire1_top = -2
        wire1_bottom = -3
        wire1_left = self.ring_loc[0] - (self.r_r_gap / 2) - self.r_core_cent + 3
        wire1_right = self.ring_loc[0] + (self.r_r_gap / 2) + self.r_core_cent - 3
        self.add_rect(layer=self.inner_electrode_ring_layers[-1],
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
        ...
        wire1_top = -1
        wire1_bottom = -2
        wire1_left = self.ring_loc[0] - (self.r_r_gap / 2) - self.r_core_cent + 3.65
        wire1_right = self.ring_loc[0] + (self.r_r_gap / 2) + self.r_core_cent - 3.65
        self.add_rect(layer=self.inner_electrode_ring_layers[-1],
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
        ...
        wire1_top = 0
        wire1_bottom = -1
        wire1_left = self.ring_loc[0] - (self.r_r_gap / 2) - self.r_core_cent + 3.9
        wire1_right = self.ring_loc[0] + (self.r_r_gap / 2) + self.r_core_cent - 3.9
        self.add_rect(layer=self.inner_electrode_ring_layers[-1],
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
    ...
    def place_contact_electrodes_basic(self):
        ...
        wire1_top = -3
        wire1_bottom = -3.5
        wire1_left = self.ring_loc[0] - (self.r_r_gap / 2) - self.r_core_cent + 1+1.4
        wire1_right = self.ring_loc[0] + (self.r_r_gap / 2) + self.r_core_cent - 1-1.4
        self.add_rect(layer=self.inner_electrode_ring_layers[-1],
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
...
if __name__ == '__main__':
    spec_file = 'Dual_ring_halfrib_point_coupler/specs/halfrib_dual_ring_vertical.yaml'
