...
from ..Single_ring_fullrib.ringheater_fullrib_spoked_dual import RingHeater
...
from ..Single_ring_fullrib.ringheater_fullrib_middle_horizontal import RingHeater_middle
...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def draw_input_wg_cena(self):
        ...
        self.extract_photonic_ports(
            inst=master_peanut,
            port_names=['PORT_IN', 'PORT_OUT_wg'],
    ...
    def draw_silicon_pieces(self):
        ...
        # add RX for bus copler

        # RX coupler piece ADDIRIOn
        self.add_rect(layer=('RX', 'drawing'),
                      bbox=BBox(right=-16.705,
                                bottom=8.5,
                                left=-20.405,
                                top=10,
                                resolution=self.grid.resolution)
    ...
    def draw_KG_pieces(self):
        ...
        # ADD kg PIECES
        self.add_rect(layer=('KG', 'drawing'),
                      bbox=BBox(right=-19.505, bottom=9.66, left=-20.105, top=9.69, resolution=self.grid.resolution))
        self.add_rect(layer=('KG', 'drawing'),
                      bbox=BBox(right=-19.505, bottom=-9.69, left=-20.105, top=-9.66, resolution=self.grid.resolution))
    ...
    def draw_heater(self):
        ...
        if not self.params['heater_disable']:
            ...
            heater_master = self.new_template(params=heater_params, temp_cls=RingHeater)
            self.add_instance(master=heater_master, loc=(self.ring_loc[0] - (self.r_r_gap  + self.core_width ) - 2*self.r_core_cent, self.ring_loc[1]))
            heater_master = self.new_template(params=heater_params, temp_cls=RingHeater_middle)
    ...
    def place_contact_electrodes(self):
        ...
        wire1_left = self.ring_loc[0] -(self.r_r_gap / 2 + self.core_width) - self.r_core_cent + offset_distance -(self.r_r_gap/2+self.core_width/2 )-self.r_core_cent
        ...
        wire1_top = self.ring_loc[1] - self.r_core_cent + offset_distance + 1
        wire1_bottom = wire1_top - 6 * offset_distance
        wire1_right = self.ring_loc[0] + (self.r_r_gap / 2 + self.core_width) + self.r_core_cent - offset_distance  +(self.r_r_gap/2+self.core_width/2 )+self.r_core_cent
    ...
    def create_in_out_taper(self):
        ...
        taper_params = {'width0': self.in_out_port_width,
                        ...
                        'width_partial': self.coup_slab_width,  # hack: here this is supposed to be self.coup_slab_width but it is not working , so self.slab_width
...
if __name__ == '__main__':
    spec_file = 'Triple_ring_fullrib/specs/full_rib_spoked_specs.yaml'
