...
from ..Single_ring_fullrib.ringheater_1_fullsi_vertical import RingHeater
...
from typing import TYPE_CHECKING, List, Union, Optional
from BPG.objects import PhotonicPolygon, PhotonicRound, PhotonicRect
from ..Single_ring_fullsi_dep.ring_rib_wg_vertical import RingRibWg_sr
...
from ..triple_rings_full_si.Ring_new.wrapped_coupler_tapeout_NEW import adiabatic_coupler_cena
from ..triple_rings_full_si.Ring_new.Adiabatic_ring_cena_new_radius import adiabatic_ring_tapeout
...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        self.peanut_width = self.params['peanut_width']
    ...
    def get_params_info(cls) -> dict:
        return dict(
            ...
            middle_rmin_in='fhf',
            middle_alpha_zero_in='dfgd',
            middle_angle_coupling='fghf',
            peanut_width='bhj',
    ...
    def get_default_param_values(cls) -> dict:
        ...
        return dict(
            ...
            angle_coupling=None,
            peanut_width=None,
            middle_alpha_zero_in=None,
            middle_angle_coupling=None,
            middle_rmin_in=None,
    ...
    def PLN_middle(self):
        self.add_rect(layer=('PLN', 'drawing'),
                      bbox=BBox(right=-12.5,
                                bottom=-2.5,
                                left=-15,
                                top=2.5,
    ...
    def draw_middle_ring(self):
        ...
        peanut_width = self.peanut_width
        ...
        middle_ring_params = dict(
            ...
            middle_rmin_in=self.params['middle_rmin_in'],
            middle_alpha_zero_in=self.params['middle_alpha_zero_in'],
            middle_angle_coupling=self.params['middle_angle_coupling'],
        )
        temp_peanut = self.new_template(params=middle_ring_params, temp_cls=adiabatic_ring_tapeout)
        ...
        print("y_shift_ring2",y_shift_ring2)
...
#spoked rings addition
        ...
        # Advanced tech parameters
            ...
            inner_electrode_info2 = self.params['inner_electrode_info2'],
            inner_electrode_info1 = self.params['inner_electrode_info1'],
            via_info2 = self.params['via_info2'],
            via_info1 = self.params['via_info1']
        )
    ...
    # waveguide routing for gratings   -2-trans_vec_peanut[0], 2*self.r_core_cent+self.ring_bus_gap+ring_pos1[1]+0.2-trans_vec_peanut[1]
        ...
        trans_vec_peanut = - peanut_top_port.center
        master_peanut.move_by(dx=trans_vec_peanut[0], dy=trans_vec_peanut[1]+ring_pos1[1]+self.r_core_cent+self.ring_bus_gap+self.params['coup_core_width']/2, unit_mode=False)
        print("Peanut y transform is {}".format(trans_vec_peanut[1]))
...
# DROP waveguide routing for gratings   -2-trans_vec_peanut[0], 2*self.r_core_cent+self.ring_bus_gap+ring_pos1[1]+0.2-trans_vec_peanut[1]
        #coupler_params = dict(layer='si_full_free',layer1=('si_full_free', 'drawing'), w=peanut_width,x=0,y=0,gap=self.ring_bus_gap_drop,wg180_radius=4,r_core_cent=self.params['r_core_cent'],coup_core_width=self.params['coup_core_width'],rmin_in=self.params['rmin_in'],alpha_zero_in=self.params['alpha_zero_in'],angle_coupling=self.params['angle_coupling'],)
        #temp_peanut = self.new_template(params=coupler_params, temp_cls=adiabatic_coupler_cena)
        #master_peanut = self.add_instance(temp_peanut, orient='R0', loc=(0, 0))
        #peanut_top_port = master_peanut.get_photonic_port('PORT_CENTER1')

        #trans_vec_peanut = - peanut_top_port.center
        #master_peanut.move_by(dx=trans_vec_peanut[0],dy=(trans_vec_peanut[1] - ring_pos1[1] - self.r_core_cent - self.ring_bus_gap_drop -self.params['coup_core_width'] / 2), unit_mode=False)
        #print("Peanut y transform is {}".format(trans_vec_peanut[1]))
    ...
    #start GSG probe wiring
        ...
        wire1_left = -20
        wire1_right = ring_pos1[0] - offset_distance + 0.1+0.5
        ...
        wire1_top = ring_pos1[1] + 7
        wire1_bottom = ring_pos1[1] + offset_distance - 0.1-0.5
        wire1_left = ring_pos1[0] - self.gs_bottom_width / 2
        wire1_right = ring_pos1[0] + self.gs_bottom_width / 2
        self.add_rect(layer=self.inner_electrode_ring_layers[-1],
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
        ...
        wire2_top = wire1_top
        wire2_bottom = wire1_top - self.gs_bottom_width
...
#Heater wiring start
    #Heater wiring here
        ...
        wire1_top = ring_pos2[1] + self.gs_bottom_width / 2-0.3
        wire1_bottom = ring_pos2[1] - self.gs_bottom_width / 2-0.3
        wire1_left = -20
        ...
        wire1_bottom = 0.6
        wire1_left = -trans_vec_peanut[0] - 4
        wire1_right = wire1_left + metal_width1
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
        ...
        wire1_top = 20
        wire1_bottom = ring_pos1[1] - self.heater_electrode_top_y_span / 2-0.4
        ...
        metal_width2=4
        # # heater wiring to the right upper pad 5
        wire1_top = 6 * offset_distance
        wire1_bottom = 0.6
        wire1_left = trans_vec_peanut[0] + 4 - metal_width2
        wire1_right = wire1_left + metal_width2
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
        ...
        # # connect middle ring BA frpm one metal to other
        wire1_top = -0.85
        wire1_bottom = -0.85 - 3.5
        wire1_left = -(trans_vec_peanut[0] + 4)
        wire1_right = trans_vec_peanut[0] + 4
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
    ...
    def draw_concentric_rings1(self, ring_info):
        ...
        y_shift_ring2=1.9434999999999993 # this is the most important polacing parameter
        ring_pos1 = (0, y_shift_ring2 + peanut_width / 2 + spoked_peanut_gap + self.r_core_cent)
        """
        ...
        for count in range(len(self.via_info1)):
            ...
            for via_count in range(via_num):
                ...
                self.add_via_stack(
                    ...
                    loc=(self.ring_loc[0] + (self.r_core_cent + self.via_info1[count]['dist']) * np.cos(phi),
                         self.ring_loc[1] + (self.r_core_cent + self.via_info1[count]['dist']) * np.sin(phi)-ring_pos2[1])
                )
