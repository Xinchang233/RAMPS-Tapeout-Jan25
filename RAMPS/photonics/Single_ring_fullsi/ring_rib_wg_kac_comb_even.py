...
from Single_ring_fullrib.ringheater_kac_comb import RingHeater
...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def draw_layout(self):
        ...
        self.place_upperpad_electrodes()
        #self.PHPN1_addition()
        self.draw_input_wg_cena()
        self.PHOTON_addition()
        self.waveguide_addition()
    ...
    def waveguide_addition(self):
        self.add_rect(layer= ('WAVEGUIDE', 'drawing'),
                      bbox=BBox(right=400,
                                bottom=-180,
                                left=-400,
                                top=180,
                                resolution=self.grid.resolution))
        self.add_rect(layer=('XG', 'drawing'),
                      bbox=BBox(right=400,
                                bottom=-180,
                                left=-400,
                                top=180,
    ...
    def draw_input_wg_cena(self):
        ...
        # Draw o(self.NN)uter portion of core
        cab=(self.NN)/2
        dt= np.linspace(-self.NN/2,self.NN/2,5)
        print(self.NN)
        if (np.mod(self.NN, 2) == 0):
            a = 0
            M = 0

            for i, gaps in (enumerate(self.gaps)):
                if (i == 0):
                    a = a + gaps / 2
                else:
                    a = a + gaps
                ...
                core_outer = self.add_round(
                    ...
                    center=((2 * i + 1) * self.r_core_cent + a, self.ring_loc[1]))
            ...
            for i, gaps in (enumerate(self.gaps)):
                ...
                if (i == 0):
                    ...
                    core_outer = self.add_round(
                        ...
                        rout=self.r_core_cent,
                        rin=self.r_core_cent - self.core_width,
                        center=((2 * i) * self.r_core_cent + a, self.ring_loc[1]))
        ...
        master_peanut.move_by(dx=trans_vec_peanut[0] - M-self.params['r_core_cent'] - self.params['coup_core_width'] / 2 - self.coup_gap  ,
        ...
        coupler_params = dict(
            ...
            wg180_radius=10,
            r_core_cent=self.params['r_core_cent'] + self.params['core_width'] / 2,
            coup_core_width=self.params['coup_core_width']+6,
            rmin_in=self.params['rmin_in'],
            alpha_zero_in=self.params['alpha_zero_in'],
            angle_coupling=self.params['angle_coupling'],
        )
        ...
        #trans_vec_peanut = - peanut_top_port.center
        #master_peanut.move_by(dx=trans_vec_peanut[0] - M - self.params['r_core_cent'] - self.params[
        #                                                                                    'coup_core_width'] / 2 - self.coup_gap,
        #                      dy=trans_vec_peanut[1],
        #                      unit_mode=False)
    ...
    def draw_fullsi_wg_ring(self):
        ...
        if (np.mod(self.NN,2) == 0):
            ...
            for i, gaps in (enumerate(self.gaps)):
                ...
                self.add_obj(core_outer)
                core_outer = self.add_round(
                    layer=('CUST_LYR50','drawing'),
                    resolution=self.grid.resolution,
                    rout=self.r_core_cent- self.core_width-1.1,
                    rin=0 ,
                    center=((2 * i + 1) * self.r_core_cent + a, self.ring_loc[1]))
                self.add_obj(core_outer)
                core_outer = self.add_round(
                ...
                if p_via_radius < n_via_radius:
                    ...
                    heater_radius = inner_pn_radius + electrode_width_p - 0.5
                else:
                    ...
                    heater_radius = inner_pn_radius + electrode_width_n - 0.5
                ...
                self.add_instance(master=heater_master, loc=((2*i+1)*self.r_core_cent+a,self.ring_loc[1]))
                self.add_instance(master=heater_master, loc=(-(2 * i + 1) * self.r_core_cent - a, self.ring_loc[1]))
            ...
            for i, gaps in (enumerate(self.gaps)):
                ...
                if (i == 0):
                    ...
                    heater_master = self.new_template(params=heater_params, temp_cls=RingHeater)
                    self.add_instance(master=heater_master, loc=((2 * i) * self.r_core_cent + a, self.ring_loc[1]))
                    self.add_instance(master=heater_master,
                                      loc=(-(2 * i ) * self.r_core_cent - a, self.ring_loc[1]))
                    self.add_instance(master=heater_master,
                                      loc=(self.ring_loc[0], self.ring_loc[1]))
    ...
    def place_heater_gs_electrodes(self):
        ...
        if (np.mod(self.NN, 2) == 0):
            print((self.NN/2)+1)
            dt = np.linspace(-self.NN +1, self.NN  -1, int((self.NN)))
            print(dt)
            for i, x in (enumerate(dt)):
                ...
                self.left_pad_loc = (self.ring_loc[0] - (dt[i])*(self.gs_pad_pitch)/2 ,
                                     self.ring_loc[1] - self.gs_pad_to_ring_dist
                                     )

                # calculate the corner coordinate of the right (signal) pad
                self.right_pad_loc = (self.ring_loc[0] + dt[i]*(self.gs_pad_pitch)/2 ,
                                      self.ring_loc[1] - self.gs_pad_to_ring_dist
                                      )
                self.add_rect(layer=self.gs_pad_layer,
                              bbox=BBox(left=self.left_pad_loc[0] - self.gs_pad_width / 2,
                ...
                self.add_rect(layer=self.gs_pad_open_layer,
                              bbox=BBox(left=self.left_pad_loc[0] - self.gs_pad_width / 2 + self.gs_pad_open_inclusion,
                                        bottom=self.left_pad_loc[
                                                   1] - self.gs_pad_length - dim1 + self.gs_pad_open_inclusion,
                                        right=self.left_pad_loc[0] + self.gs_pad_width / 2 - self.gs_pad_open_inclusion,
                                        top=self.left_pad_loc[1] - dim1 - 3,
                                        resolution=self.grid.resolution)
                              )
                ...
                self.add_rect(layer=self.gs_pad_open_layer,
                              bbox=BBox(left=self.right_pad_loc[0] - self.gs_pad_width / 2 + self.gs_pad_open_inclusion,
                                        bottom=self.left_pad_loc[
                                                   1] - self.gs_pad_length - dim1 + self.gs_pad_open_inclusion,
                                        right=self.right_pad_loc[
                                                  0] + self.gs_pad_width / 2 - self.gs_pad_open_inclusion,
                                        top=self.left_pad_loc[1] - dim1 - 3,
                                        resolution=self.grid.resolution)
                              )
            ...
            dt = np.linspace(-self.NN + 1, self.NN - 1, int((self.NN)))
            for i, x in (enumerate(dt)):
                self.heaterpad1_loc = (self.ring_loc[0] - (self.gs_pad_pitch)*dt[i]/2, self.ring_loc[1] + self.heater_pad_to_ring_dist)
                self.heaterpad2_loc = (self.ring_loc[0] + (self.gs_pad_pitch)*dt[i]/2, self.ring_loc[1] + self.heater_pad_to_ring_dist)
            ...
            print((self.NN / 2) + 1)
            dt = np.linspace(-self.NN , self.NN , int((self.NN+1)))
            print(dt)
            for i, x in (enumerate(dt)):
                dim1 = 11.7
                dim2 = dim1  # 10.722 + 6.72 - 5.812
            ...
            dt = np.linspace(-self.NN + 2, self.NN - 2, int((self.NN-1)))
            for i, x in (enumerate(dt)):
                self.heaterpad1_loc = (
                self.ring_loc[0] - (self.gs_pad_pitch) * dt[i] / 2, self.ring_loc[1] + self.heater_pad_to_ring_dist)
                self.heaterpad2_loc = (
                self.ring_loc[0] + (self.gs_pad_pitch) * dt[i] / 2, self.ring_loc[1] + self.heater_pad_to_ring_dist)
                ...
                self.add_rect(layer=self.gs_pad_open_layer,
                              bbox=BBox(
                                  left=self.heaterpad1_loc[0] - self.gs_pad_width / 2 + self.gs_pad_open_inclusion,
                                  bottom=self.heaterpad1_loc[1] + dim1 + 3,
                                  right=self.heaterpad1_loc[0] + self.gs_pad_width / 2 - self.gs_pad_open_inclusion,
                                  top=self.heaterpad1_loc[1] + self.gs_pad_length + dim1 - self.gs_pad_open_inclusion,
                                  resolution=self.grid.resolution)
                ...
                self.add_rect(layer=self.gs_pad_open_layer,
                              bbox=BBox(
                                  left=self.heaterpad2_loc[0] - self.gs_pad_width / 2 + self.gs_pad_open_inclusion,
                                  bottom=self.heaterpad2_loc[1] + dim1 + 3,
                                  right=self.heaterpad2_loc[0] + self.gs_pad_width / 2 - self.gs_pad_open_inclusion,
                                  top=self.heaterpad2_loc[1] + self.gs_pad_length + dim1 - self.gs_pad_open_inclusion,
                                  resolution=self.grid.resolution)
    ...
    def draw_heater(self):
        ...
        if p_via_radius < n_via_radius:
            ...
            electrode_rin_n = n_via_radius + self.electrode_offset
            outer_pn_radius = electrode_rin_n + electrode_width_n
            heater_radius = inner_pn_radius + electrode_width_p-0.3
        else:
            ...
            electrode_rout_n = n_via_radius - self.electrode_offset
            inner_pn_radius = electrode_rout_n - electrode_width_n
            heater_radius = inner_pn_radius + electrode_width_n-0.3
        offset_distance = outer_pn_radius
    ...
    def draw_input_wg(self):
        ...
        master_peanut.move_by(dx=trans_vec_peanut[0],
                              dy=trans_vec_peanut[1] + ring_bus_gap+self.r_core_cent+self.params['coup_core_width']/2 ,
                              unit_mode=False)
        x_coord=trans_vec_peanut[0]
        y_coord=-(trans_vec_peanut[1]+ ring_bus_gap+self.params['coup_core_width']/2)
        print("Peanut x transform(for lumerical) is {}".format(x_coord))
        print("Peanut y transform(for lumerical) is {}".format(y_coord))

    def draw_wrapped_input_wg(self):
        """
        Draws input coupler waveguide of the rib-waveguide ring.
        """
        position_ring1 = (0, 0)
        ...
        trans_vec_peanut = - peanut_top_port.center
        master_peanut.move_by(dx=trans_vec_peanut[0],
                              dy=trans_vec_peanut[1] +self.coup_gap+self.r_core_cent+self.params['coup_core_width']/2+self.core_width/2 ,
                              unit_mode=False)
    ...
    def draw_input_wg_drop(self):
        ...
        trans_vec_peanut = - peanut_top_port.center
        master_peanut.move_by(dx=trans_vec_peanut[0],
                              dy=trans_vec_peanut[1] - self.ring_bus_gap_drop-self.r_core_cent-self.params['coup_core_width']/2 ,
                              unit_mode=False)
        x_coord=trans_vec_peanut[0]
        y_coord=(trans_vec_peanut[1] - self.ring_bus_gap_drop-self.r_core_cent-self.params['coup_core_width']/2)
        print("Peanut x transform(for lumerical) is {}".format(x_coord))
        print("Peanut y transform(for lumerical) is {}".format(y_coord))
    ...
    def place_contact_electrodes(self):
        ...
        dim2 = dim1  # 10.722 + 6.72 - 5.812
        # left pad locations
        self.heaterpadleft_loc1 = (
            self.ring_loc[0] - (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)
        self.heaterpadleft_loc2 = (
            self.ring_loc[0] - 3 * (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)
        self.heaterpadleft_loc3 = (
            self.ring_loc[0] - 5 * (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)
        self.heaterpadleft_loc4 = (
            self.ring_loc[0] - 7 * (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)
        self.heaterpadleft_loc5 = (
            self.ring_loc[0] - 9 * (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)

        self.heaterpadright_loc1 = (
            self.ring_loc[0] + (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)
        self.heaterpadright_loc2 = (
            self.ring_loc[0] + 3 * (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)
        self.heaterpadright_loc3 = (
            self.ring_loc[0] + 5 * (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)
        self.heaterpadright_loc4 = (
            self.ring_loc[0] + 7 * (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)
        self.heaterpadright_loc5 = (
            self.ring_loc[0] + 9 * (self.gs_pad_pitch) / 2, self.ring_loc[1] - self.heater_pad_to_ring_dist)

        # set of right side rings
        ring_rightloc1 = (self.gaps[0] / 2 + self.r_core_cent, self.ring_loc[1])
        ring_rightloc2 = (ring_rightloc1[0] + 2 * self.r_core_cent + self.gaps[1], self.ring_loc[1])
        ring_rightloc3 = (ring_rightloc2[0] + 2 * self.r_core_cent + self.gaps[2], self.ring_loc[1])
        ring_rightloc4 = (ring_rightloc3[0] + 2 * self.r_core_cent + self.gaps[3], self.ring_loc[1])
        ring_rightloc5 = (ring_rightloc4[0] + 2 * self.r_core_cent + self.gaps[4], self.ring_loc[1])

        # set of left side rings

        ring_leftloc1 = (-self.gaps[0] / 2 - self.r_core_cent, self.ring_loc[1])
        ring_leftloc2 = (ring_leftloc1[0] - 2 * self.r_core_cent - self.gaps[1], self.ring_loc[1])
        ring_leftloc3 = (ring_leftloc2[0] - 2 * self.r_core_cent - self.gaps[2], self.ring_loc[1])
        ring_leftloc4 = (ring_leftloc3[0] - 2 * self.r_core_cent - self.gaps[3], self.ring_loc[1])
        ring_leftloc5 = (ring_leftloc4[0] - 2 * self.r_core_cent - self.gaps[4], self.ring_loc[1])

        offset_distance = 25
        right_metal_layer = ('BA', 'drawing')
        metal_width = 2

        # heater wiring to the left 5 pad to left array ring 4-(left contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom =  -offset_distance
        wire1_left = ring_leftloc4[0] - self.contact_dist / 2 - metal_width
        ...
        wire2_left = self.heaterpadleft_loc5[0]
        ...
        wire3_top = wire2_top
        wire3_bottom = self.heaterpadleft_loc5[1] - dim1 / 2
        ...
        # heater wiring to the left 3 pad to left array ring 2-(left contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -2*offset_distance
        wire1_left = ring_leftloc2[0] - self.contact_dist / 2 - metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire2_top = wire1_bottom + metal_width
        wire2_bottom = wire1_bottom
        wire2_left = self.heaterpadleft_loc3[0]
        wire2_right = wire1_right
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
        ...
        wire3_top = wire2_top
        wire3_bottom = self.heaterpadleft_loc3[1] - dim1 / 2
        wire3_left = wire2_left
        wire3_right = wire2_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire3_right,
        ...
        # heater wiring to the left 4 pad to left array ring 4-(left contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -1.5 * offset_distance
        wire1_left = ring_leftloc4[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
                                top=wire1_top,
        ...
        wire2_top = wire1_bottom + metal_width
        wire2_bottom = wire1_bottom
        wire2_left = self.heaterpadleft_loc4[0]
        wire2_right = wire1_right
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
        ...
        wire3_top = wire2_top
        wire3_bottom = self.heaterpadleft_loc4[1] - dim1 / 2
        wire3_left = wire2_left
        wire3_right = wire2_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire3_right,
        ...
        # heater wiring to the left 2 pad to left array ring 2-(left contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -2.5 * offset_distance
        wire1_left = ring_leftloc2[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire2_top = wire1_bottom + metal_width
        wire2_bottom = wire1_bottom
        wire2_left = self.heaterpadleft_loc2[0]
        wire2_right = wire1_right
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
        ...
        wire3_top = wire2_top
        wire3_bottom = self.heaterpadleft_loc2[1] - dim1 / 2
        wire3_left = wire2_left
        wire3_right = wire2_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire3_right,
        ...
        # heater wiring to the left 1 pad to right array ring 1-(left contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -2 * offset_distance
        wire1_left = ring_rightloc1[0] - self.contact_dist / 2 - metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire2_top = wire1_bottom + metal_width
        wire2_bottom = wire1_bottom
        wire2_left = self.heaterpadleft_loc1[0]
        wire2_right = wire1_right
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
        ...
        wire3_top = wire2_top
        wire3_bottom = self.heaterpadleft_loc1[1] - dim1 / 2
        wire3_left = wire2_left
        wire3_right = wire2_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire3_right,
        ...
        # heater wiring to the right 1 pad to right array ring 1-(right contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -2 * offset_distance
        wire1_left = ring_rightloc1[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire2_right = self.heaterpadright_loc1[0]
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
                                left=wire2_left,
        ...
        # heater wiring to the right 3 pad to right array ring 3-(right contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -2 * offset_distance
        wire1_left = ring_rightloc3[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire2_bottom = wire1_bottom
        wire2_left = wire1_left
        wire2_right = self.heaterpadright_loc3[0]
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
                                left=wire2_left,
        ...
        # heater wiring to the right 5 pad to right array ring 5-(right contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -2 * offset_distance
        wire1_left = ring_rightloc5[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire2_bottom = wire1_bottom
        wire2_left = wire1_left
        wire2_right = self.heaterpadright_loc5[0]
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
                                left=wire2_left,
        ...
        # heater wiring to the right 2 pad to right array ring 3-(left contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -1.5 * offset_distance
        wire1_left = ring_rightloc3[0] - self.contact_dist / 2 - metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire2_bottom = wire1_bottom
        wire2_left = wire1_left
        wire2_right = self.heaterpadright_loc2[0]
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
                                left=wire2_left,
        ...
        # heater wiring to the right 4 pad to right array ring 4-(left contact)
        wire1_top = -self.heater_electrode_top_y_span / 2
        wire1_bottom = -1.5 * offset_distance
        wire1_left = ring_rightloc5[0] - self.contact_dist / 2 - metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire2_bottom = wire1_bottom
        wire2_left = wire1_left
        wire2_right = self.heaterpadright_loc4[0]
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
                                left=wire2_left,
    ...
    def place_upperpad_electrodes(self):
        # [-5,-4,-3,-2,-1,1,2,3,,4,5]
...
# set of left side rings
        ...
        # heater wiring to the left 1 pad to left array ring 1-(left contact)
        wire1_top =  offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_leftloc1[0]-self.contact_dist / 2 - metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire3_top = self.heaterpadleft_loc1[1] + dim1 / 2
        ...
        # heater wiring to the left 3 pad to left array ring 3-(left contact)
        wire1_top = 2*offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_leftloc3[0] - self.contact_dist / 2 - metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire3_top = self.heaterpadleft_loc3[1] + dim1 / 2
        wire3_bottom = wire2_bottom
        wire3_left = wire2_left
        wire3_right = wire2_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
        ...
        # heater wiring to the left 5 pad to left array ring 5-(left contact)
        wire1_top = 2 * offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_leftloc5[0] - self.contact_dist / 2 - metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire3_top = self.heaterpadleft_loc5[1] + dim1 / 2
        wire3_bottom = wire2_bottom
        wire3_left = wire2_left
        wire3_right = wire2_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
        ...
        # heater wiring to the left 4 pad to left array ring 5-(right contact)
        wire1_top =  offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_leftloc5[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire3_top = self.heaterpadleft_loc4[1] + dim1 / 2
        wire3_bottom = wire2_bottom
        wire3_left = wire2_left
        wire3_right = wire2_left + metal_width
        self.add_rect(layer=right_metal_layer,
        ...
        # heater wiring to the left 2 pad to left array ring 3-(right contact)
        wire1_top = 2 * offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_leftloc3[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire3_top = self.heaterpadleft_loc2[1] + dim1 / 2
        wire3_bottom = wire2_bottom
        wire3_left = wire2_left
        wire3_right = wire2_left + metal_width
        self.add_rect(layer=right_metal_layer,
        ...
        # heater wiring to the right 1 pad to left array ring 1-(right contact)


        wire1_top =  offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_leftloc1[0]+self.contact_dist / 2
        wire1_right = wire1_left  + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        # heater wiring to the right 3 pad to left array ring 2-(right contact)


        wire1_top = 2*offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_rightloc2[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire3_top = self.heaterpadright_loc3[1] + dim1 / 2
        wire3_bottom = wire2_bottom
        wire3_left = wire2_right - metal_width
        wire3_right = wire2_right
        self.add_rect(layer=right_metal_layer,
        ...
        # heater wiring to the right 5 pad to left array ring 4-(right contact)


        wire1_top = offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_rightloc4[0] + self.contact_dist / 2
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=right_metal_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire3_top = self.heaterpadright_loc5[1] + dim1 / 2
        wire3_bottom = wire2_bottom
        wire3_left = wire2_right - metal_width
        wire3_right = wire2_right
        self.add_rect(layer=right_metal_layer,
        ...
        # heater wiring to the right 2 pad to left array ring 2-(left contact)


        wire1_top = 2 * offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_rightloc2[0] - self.contact_dist / 2-metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        # heater wiring to the right4 pad to left array ring 4-(left contact)


        wire1_top =   offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span / 2
        wire1_left = ring_rightloc4[0] - self.contact_dist / 2 - metal_width
        wire1_right = wire1_left + metal_width
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
        ...
        wire3_top = self.heaterpadright_loc4[1] + dim1 / 2
        wire3_bottom = wire2_bottom
        wire3_left = wire2_right - metal_width
        wire3_right = wire2_right
        self.add_rect(layer=self.heater_electrode_top_layer,
    ...
    # def draw_heater_ring(self):
        ...
        spoke_width_percentage = 2 * self.photonic_tech_info.min_width(self.layer) / (
            np.pi * rin / spoke_num) + self.si_spoke_offset_percentage # earlier it was 0.05
...
if __name__ == '__main__':
    #spec_file = 'Single_ring_fullsi/specs/fullsi_single_ring_spoked.yaml'
    spec_file = 'Single_ring_fullsi/specs/fullsi_single_ring_kac_comb_even.yaml'
