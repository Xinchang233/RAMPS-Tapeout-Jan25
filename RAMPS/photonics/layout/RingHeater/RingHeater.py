...
class RingHeater(BPG.PhotonicTemplateBase):
    """
    Base ring-heater class draws a ring heater with 2 spokes that at its ends have via stack instantiated up to a desired
    metal layer. Labels are places at the contacts on the top metal.

    Mofified on 12/20/2021 by kramnik to use same electrode layer names as source ring test site and add pin_primitives
    """
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        # Layer information
        #self.electrode_bottom_lpp = self.params['electrode_layer_bottom']
        #self.electrode_top_lpp = self.params['electrode_layer_top']
    ...
    def get_params_info(cls):  # Returns definition of the parameters.
        return dict(
            ...
            electrode_layer_top='LPP of top metal layer of the gs electrode',
            ...
            electrode_layer_bottom='LPP for the bottom electrode layer in the stack (should be the BAG '
    ...
    def draw_layout(self):
        ...
        viastack_params = dict(
            top_layer = self.electrode_layer_top,
            bottom_layer = self.electrode_layer_bottom,
        ...
        viastack_master = self.new_template( params = viastack_params, temp_cls = ViaStack )

        via_stack_x_offset = self.contact_dist / 2.0 + self.electrode_bottom_x_span / 2.0

        # Place the via stacks
        left_heater_loc = ( -via_stack_x_offset, 0 )
        self.viastack_left_inst = self.add_instance(
            master = viastack_master,
        ...
        right_heater_loc = ( via_stack_x_offset, 0 )
        self.viastack_right_inst = self.add_instance(
            master = viastack_master,
        ...
        # electrode_top_rect: PhotonicRect = viastack_master.top_layer_rect
        ...
        self.add_pin_primitive(
            net_name = self.electrode_label_p,
            label = self.electrode_label_p,
            layer = self.electrode_layer_top[ 0 ],
            bbox = self.viastack_left_inst.bound_box,
        )

        print( self.viastack_left_inst.bound_box )

        self.add_pin_primitive(
            net_name = self.electrode_label_n,
            label = self.electrode_label_n,
            layer = self.electrode_layer_top[ 0 ],
            bbox = self.viastack_right_inst.bound_box,
        )
