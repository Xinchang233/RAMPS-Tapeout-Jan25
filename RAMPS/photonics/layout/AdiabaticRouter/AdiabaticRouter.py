...
from layout.BasicElements.ParabolicPolygon.ParabolicPolygon import ParabolicPolygon
from layout.BasicElements.SplineBend.SplineBend import SplineBend
...
class AdiabaticRouter:
    ...
    def add_circular_bend(self, rout: float = 0,
                          theta_in: float = 0,
                          theta_out: float = 360,
                          width=None):
        ...
        bend_params = dict(layer=self.layer,
        ...
        temp = self.gen_cls.new_template(params=bend_params, temp_cls=SimpleRing)
        index = len(self.inst)
        self.inst[index] = self.gen_cls.add_instance_port_to_port(inst_master=temp, instance_port_name='PORT_IN',
                                                                  self_port=self.port)
        self.port = self.inst[index]['PORT_OUT']
