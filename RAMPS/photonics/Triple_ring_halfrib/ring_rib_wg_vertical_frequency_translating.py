...
from Single_ring_fullrib.ringheater_halfrib_vertical_freq_translating import RingHeater
...
from triple_rings_full_si.Ring_new.wrapped_coupler_tapeout_frequency_translating import adiabatic_coupler_cena
from triple_rings_full_si.Ring_new.wrapped_coupler_tapeout_frequency_translating_heater import adiabatic_coupler_cena_heater
...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        self.coup_gap_middlering = self.params['coup_gap_middlering']
        self.coup_core_width_middlering = self.params['coup_core_width_middlering']
    ...
    def get_params_info(cls) -> dict:
        return dict(
            # Parameters of rib-waveguide ring
            # --------------------------------
            coup_core_width_middlering='df',
            coup_gap_middlering='gfh',
    ...
    def get_default_param_values(cls) -> dict:
        ...
        """
        return dict(
            coup_core_width_middlering=None,
            coup_gap_middlering=None,
            r_core_cent=None,
            r_r_gap=None,
            core_layer=None,
            core_width=None,
    ...
    def draw_layout(self):
        ...
        self.draw_input_wg_cena_frequency_translating()
        ...
        self.PHPN1_addition()
        self.draw_input_wg_middle_ring()
        self.draw_input_wg_heater()
        self.draw_input_wg_heater_nnphot()
        self.coupler_metal_via_contacts()
        self.place_heater_UA_LAYER()
        self.photon_addition1()
        self.nnphot_addition1()
    ...
    def nnphot_addition1(self):
        self.add_rect(layer= ('n_heavy_sil', 'drawing'),
                      bbox=BBox(right=12,
                                bottom=10,
                                left=-12,
                                top=16,
    ...
    def draw_input_wg_cena_frequency_translating(self):
        """
        ...
        peanut_top_port = master_peanut.get_photonic_port('PORT_180LEFT_BOT')
    ...
    def draw_input_wg_heater(self):
        ...
        coupler_params = dict(
            ...
            wg180_radius=10,
            r_core_cent=self.params['r_core_cent'] + self.params['core_width'] / 2,
            coup_core_width=2.5,#self.params['coup_core_width']
            rmin_in=self.params['rmin_in'],
            alpha_zero_in=self.params['alpha_zero_in'],
            angle_coupling=self.params['angle_coupling'],
            core_width = self.params['core_width'],
        ...
        temp_peanut = self.new_template(params=coupler_params, temp_cls=adiabatic_coupler_cena_heater)
        master_peanut = self.add_instance(temp_peanut, orient='R90', loc=(0, 0))
        peanut_top_port = master_peanut.get_photonic_port('PORT_OUT_FINAL')

        trans_vec_peanut = - peanut_top_port.center
        master_peanut.move_by(dx=trans_vec_peanut[0]-11  ,
           dy=trans_vec_peanut[1]+12,unit_mode=False)
    ...
    def draw_input_wg_heater_nnphot(self):
        """
        Draws input coupler waveguide of the rib-waveguide ring.
        """
        position_ring1 = (0, 0)
    ...
    def draw_input_wg_middle_ring(self):
        """
        Draws input coupler waveguide of the rib-waveguide ring.
        """
        ...
        bent_coup_params = {'arc_params':
                            [{'arc_type': 'wrapped_ring_coupler',
                              ...
                              'w_in': self.coup_core_width_middlering,
        ...
        if self.bent_coupler:
            ...
            loc = (self.ring_loc[0] - self.coup_length / 2,
                   self.ring_loc[1] - self.r_core_cent - self.coup_gap - (self.core_width + self.coup_core_width_middlering) / 2)
    ...
    #heater_contacts_for the outer coupler
    def coupler_metal_via_contacts(self):

        viastack_params = dict(
            top_layer=self.params['heater_electrode_top_layer'],
            bottom_layer=self.params['heater_electrode_bottom_layer'],
            top_x_span=self.params['heater_electrode_top_x_span'],
            top_y_span=self.params['heater_electrode_top_y_span'],
            bottom_x_span=self.params['heater_electrode_bottom_x_span'],
            bottom_y_span=self.params['heater_electrode_bottom_y_span'],
            side_align=True,
        )
        ...
        left_heater_loc = (-self.contact_dist / 2.0-3, via_coord_y)
        self.add_instance(
        ...
        right_heater_loc = (self.contact_dist / 2.0+3, via_coord_y)
        self.add_instance(
            master=electrode_master,
        ...
        self.add_rect(layer=self.params['heater_electrode_bottom_layer'],
                      bbox=BBox(right=-self.contact_dist / 2.0-3,
                                bottom=via_coord_y-0.25,
                                left=-11,
                                top=via_coord_y+0.25,
                                resolution=self.grid.resolution)
                      )
        self.add_rect(layer=self.params['heater_electrode_bottom_layer'],
                      bbox=BBox(right=11,
                                bottom=via_coord_y - 0.25,
                                left=self.contact_dist / 2.0 +3 ,
                                top=via_coord_y + 0.25,
                                resolution=self.grid.resolution)
                      )
    ...
    def draw_rib_wg_ring3(self):
        ...
        slab_inner = self.add_round(
            ...
            resolution=self.grid.resolution,
            rin=self.r_core_cent - self.slab_width / 2+0.1,
    ...
    def draw_heater(self):
        ...
        if not self.params['heater_disable']:
            # Compute the width from the heater resistance design function
            heater_middle_params['width'] = RingHeater.design_heater_width_from_resistance(
                resistance=heater_middle_params['resistance'],
                heater_rout=heater_middle_params['rout'],
                r_square=self.photonic_tech_info.sheet_resistance(heater_middle_params['device_layer']),
                contact_width=heater_middle_params['contact_width'],
                contact_dist=heater_middle_params['contact_dist'],
            )
    ...
    def place_heater_contact_electrodes(self):
        ...
        dim2 = dim1  # 10.722 + 6.72 - 5.812
        self.heaterpad1_loc = (
            self.ring_loc[0] - 2 * (self.gs_pad_pitch) , self.ring_loc[1] + self.heater_pad_to_ring_dist)
        self.heaterpad2_loc = (
            self.ring_loc[0] - 1 * (self.gs_pad_pitch) , self.ring_loc[1] + self.heater_pad_to_ring_dist)
        self.heaterpad3_loc = (
            self.ring_loc[0] , self.ring_loc[1] + self.heater_pad_to_ring_dist)
        self.heaterpad4_loc = (
            self.ring_loc[0] + 1*(self.gs_pad_pitch) , self.ring_loc[1] + self.heater_pad_to_ring_dist)
        self.heaterpad5_loc = (
            self.ring_loc[0] + 2 * (self.gs_pad_pitch) , self.ring_loc[1] + self.heater_pad_to_ring_dist)
        ...
        wire1_top = 6 * offset_distance
        wire1_bottom = 13.25
        wire1_left = self.contact_dist / 2 +3
        wire1_right = self.contact_dist / 2 + metal_width+3
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
    ...
    def place_heater_UA_LAYER(self):
        # calculate the corner coordinate of the left (ground) pad
        # calculate the corner coordinate of the left (ground) pad
        dim1 = 11.7
        dim2 = dim1  # 10.722 + 6.72 - 5.812
        ...
        # heater wiring upto middle ring with UA
        wire1_top = self.heaterpad1_loc[1] + dim1 / 2
        wire1_bottom = 0
        ...
        # # heater wiring upto left  ring contact with UA
        ...
        wire2_top = wire1_top
        wire2_bottom = wire1_top - metal_width
        wire2_left = wire1_right
        wire2_right = self.heaterpad3_loc[0]
        self.add_rect(layer=('UA','drawing'),
                      bbox=BBox(right=wire2_right,
                                bottom=wire2_bottom,
                                left=wire2_left,
        ...
        # # heater wiring upto right  ring contact with UA
        wire1_top = 9 * offset_distance
        wire1_bottom = 0
        wire1_left = -self.contact_dist / 2 + (self.r_r_gap + self.core_width) + 2 * self.r_core_cent-metal_width
        wire1_right = wire1_left + metal_width
        ...
        # # inter heater wiring upto contact with coupler heater with UA
        wire1_top = 13.25+ metal_width/2
        wire1_bottom = 13.25- metal_width/2
        wire1_left = self.contact_dist / 2 - (self.r_r_gap + self.core_width) - 2 * self.r_core_cent
        wire1_right = -self.contact_dist / 2 - 3
        self.add_rect(layer=('UA', 'drawing'),
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
...
if __name__ == '__main__':
    spec_file = 'Triple_ring_halfrib/specs/half_rib_triple_ring_vertical_frequency_translating.yaml'
