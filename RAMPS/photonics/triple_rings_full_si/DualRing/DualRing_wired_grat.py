...
class DualRing(BPG.PhotonicTemplateBase):
    ...
    def draw_layout(self) -> None:
        ...
        grat_params = dict(
            grR1r_port=master_rings[0]['PORT0'],
            grR1l_port=master_rings[0]['PORT1'],
            grR2r_port=master_rings[1]['PORT1'],
            grR2l_port=master_rings[1]['PORT0'],
            mod_y_diff=mod_y_diff,
            ring_acces_wds=ring_acces_wds,
            # master_rings=master_rings,
        )

        temp_grats = self.new_template(params=grat_params, temp_cls=LNAgrat)
        master_grats = self.add_instance(temp_grats, orient='R0', loc=(0, 0))
