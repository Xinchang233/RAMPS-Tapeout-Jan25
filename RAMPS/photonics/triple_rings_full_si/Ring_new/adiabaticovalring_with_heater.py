...
from layout.Ring_new.GS_pads1_ovalring import GSPads1
...
#         gap = 0.3
#         wg180_radius = 5
        init_port11 = self.add_photonic_port(name='init_port11', center=(15,  self.coupling_slot_drop + self.wg_width / 2 -self.placer/2+0.7), orient='R180',
...
#                              name='init_port11')
#         Wg.add_straight_wg(length=50, width=0.5)
#         Wg.add_offset_bend(offset=155, rmin=20)
#         Wg.add_straight_wg(length=215, width=0.5)
...
#             reflect=False)
#
        # init_port111 = self.add_photonic_port(name='init_port111', center=(-15, self.coupling_slot_drop + self.wg_width / 2 - self.placer / 2 + 0.7), orient='R0',
        #                                      width=self.w, layer=('rxphot_noopc', 'port'))
        # #         # init_port = self.add_photonic_port(name='init_port', center=(0,0), orient='R0',width=w, layer=('RX', 'port'))
        # #         # (27.6/2,5.6-2.4275919117522138)
        # Wg = AdiabaticRouter(gen_cls=self,
...
#             reflect=False)
        ...
        init_port111 = self.add_photonic_port(name='init_port111', center=(22.429/2-0.2, 0), orient='R90',
                                             width=self.w, layer=('rxphot_noopc', 'port'))
        #         # init_port = self.add_photonic_port(name='init_port', center=(0,0), orient='R0',width=w, layer=('RX', 'port'))
        #         # (27.6/2,5.6-2.4275919117522138)
        Wg = AdiabaticRouter(gen_cls=self,
                             ...
                             layer=('rxphot_noopc', 'drawing'),
                             name='init_port111')
        Wg.add_bend_180(rmin=8, turn_left=False, width=self.w)
        Wg.add_bend_180(rmin=8, turn_left=False, width=self.w)
        ...
        #                                  'theta1': 90},
        #                          temp_cls=SimpleRound)
        # self.add_instance(master=ring, loc=[4, 0], inst_name='ring')
        #
        # ring = self.new_template(params={'layer': ('rxphot_noopc', 'drawing'),
        #                                  'r_out': 5.6,
        #                                  'r_width': 0.6,
