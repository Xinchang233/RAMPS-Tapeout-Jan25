...
class RingHeater(BPG.PhotonicTemplateBase):
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        # Layer information
        self.electrode_bottom_lpp = ('RXbar','drawing')
    ...
    def draw_layout(self):
        ...
        left_heater_loc = (-self.contact_dist -0.5, self.contact_width/2 -0.5)#(-self.contact_dist/2.0, 0)
        #left_heater_loc = (-self.contact_dist , self.contact_width / 2 )
        self.add_instance(
        ...
        right_heater_loc = (self.contact_dist +0.5, self.contact_width/2 -0.5)
        #right_heater_loc = (self.contact_dist , self.contact_width / 2 )
        self.add_instance(
            master=electrode_master,
        ...
        leftd_heater_loc = (-self.contact_dist - 1.5, self.contact_width / 2 - 1.0)
        rightd_heater_loc = (self.contact_dist + 1.5, self.contact_width / 2 - 1.0)
        leftu_heater_loc = (-self.contact_dist - 1.5, self.contact_width / 2 + 1.0)
        rightu_heater_loc = (self.contact_dist + 1.5, self.contact_width / 2 + 1.0)

        contact_points = [(rightd_heater_loc[0],rightd_heater_loc[1]-1),
                          (leftd_heater_loc[0],leftd_heater_loc[1]-1),
                          (leftd_heater_loc[0],leftd_heater_loc[1]+1),
                          (rightd_heater_loc[0], rightd_heater_loc[1]+1)
                          ]
        polygon = PhotonicPolygon(
            resolution=self.grid.resolution,
            layer=self.electrode_top_lpp,
    ...
    def draw_heater_ring_and_contacts(self,
        ...
        # adiabatic_params =
        heater_rin = heater_rout - ring_width/2
        bend180_params = dict(layer=device_layer, port_layer=['RX', 'port'], radius_threshold=1.5,
                                     curvature_rate_threshold=0.7, merge_arcs=False)
        bend180_params['arc_params'] = [
            dict(arc_type="180_bend", size=heater_rin*2, turn_left=False, width=[ring_width])]
        bend180_temp = self.new_template(params=bend180_params, temp_cls=AdiabaticPaths)
        bend180_master = self.add_instance(master=bend180_temp, orient='R90', loc=(-heater_rin,0))
        ...
        #                   ]
        # contact_points = [(-heater_rin, 0),
        #                   (-contact_dist / 2.0, 0),
        #                   (-contact_dist / 2.0, contact_width),
        #                   (-heater_rin, contact_width)
        #                   ]
        contact_points = [(-heater_rin, 0),
                          (-contact_dist / 2.0, 0),
                          (-contact_dist / 2.0, contact_width),
        ...
        if disk_layers:
            for layer in disk_layers:
                ...
                bend180_params2 = deepcopy(bend180_params)
                bend180_params2['layer'] = layer
                bend180_params2['arc_params'] = [
                    dict(arc_type="180_bend", size=heater_rin * 2, turn_left=False, width=[ring_width+self.disk_layer_extension])]
                bend180_temp2 = self.new_template(params=bend180_params2, temp_cls=AdiabaticPaths)
                bend180_master2 = self.add_instance(master=bend180_temp2, orient='R90', loc=(-heater_rin, 0))

                contact_points = [(heater_rout + self.disk_layer_extension, -  self.disk_layer_extension),
                                  (-heater_rout - self.disk_layer_extension, - self.disk_layer_extension),
                                  (-heater_rout - self.disk_layer_extension, contact_width + self.disk_layer_extension+1),
                                  (heater_rout + self.disk_layer_extension, contact_width + self.disk_layer_extension+1)
                                  ]
...
if __name__ == '__main__':
    spec_file = 'Photonic_Core_Layout/Ring/specs/ringheater_specs.yaml'
