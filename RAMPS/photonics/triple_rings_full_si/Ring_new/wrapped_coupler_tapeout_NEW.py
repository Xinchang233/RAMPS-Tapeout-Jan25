...
#(5+5*math.sin(pi/6)+5.6*math.sin(pi/5), 5.6-(5.6-5.6*math.cos(pi/5)+5-5*math.cos(pi/5)))
    def draw_layout(self) -> None:
        pi=np.pi
        init_port = self.add_photonic_port(name='init_port', center=((self.x + 30) / 2, -(self.r_core_cent + self.coup_core_width/2 + self.gap - self.y)),
                                           orient='R0',
                                           width=0.35, layer=self.layer)
        ...
        Wg = AdiabaticRouter(gen_cls=self,
                             ...
                             name='init_port')
        Wg.add_straight_wg(length=15,width=self.coup_core_width)
        ...
        # print(a, X, Y)
        Wg.add_straight_wg(length=15,width=0.35)
