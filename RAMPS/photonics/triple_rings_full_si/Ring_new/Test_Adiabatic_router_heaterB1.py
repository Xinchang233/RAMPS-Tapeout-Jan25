...
import numpy as np
import numpy as np
from Photonic_Core_Layout_Djordje.Ring.ringheater_theta2 import RingHeater

class adiabatic_ring(BPG.PhotonicTemplateBase):
...
#(5+5*math.sin(pi/6)+5.6*math.sin(pi/5), 5.6-(5.6-5.6*math.cos(pi/5)+5-5*math.cos(pi/5)))
    ...
    def place_heater_ring(self,ubend_span):
        ...
        heater_electrode_top_layer = ('B1', 'drawing')
        ...
        if heater_electrode_bottom_y_span is None:
            ...
            heater_params['width'] = 5.0 # heater width
