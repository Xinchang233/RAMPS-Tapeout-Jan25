...
import numpy as np
import numpy as np
from ...triple_rings_full_si.Ring_new.ringheater_theta2 import RingHeater
...
#(5+5*math.sin(pi/6)+5.6*math.sin(pi/5), 5.6-(5.6-5.6*math.cos(pi/5)+5-5*math.cos(pi/5)))
    def draw_layout(self) -> None:
        pi=np.pi
        init_port = self.add_photonic_port(name='init_port', center=((self.x + 2) / 2, -(self.r_core_cent + self.w/2 + self.gap - self.y)),
                                           orient='R0',
        ...
        Wg = AdiabaticRouter(gen_cls=self,
                             ...
                             name='init_port')
        Wg.add_straight_wg(length=connection)
        a1 = len(Wg.inst)
        print("a=", a1)
        #self.layer1[0]
        ...
        a2 = compute_arc_length(Wg.inst[1].master.arc_list)
        ...
        a4 = compute_arc_length(Wg.inst[3].master.arc_list)
        print("a=", a4)
        self.extract_photonic_ports(
            inst=Wg.inst[list(Wg.inst)[-1]],
        ...
        Wg.add_wrapped_ring_coupler_centport(rmin_in=self.middle_rmin_in, alpha_zero_in=self.middle_alpha_zero_in* np.pi/180,
                                             r_coupling=self.r_core_cent + self.w/2 + self.gap,
                                             angle_coupling=self.middle_angle_coupling*np.pi/180, w_coupling=self.w)
        a6 = compute_arc_length(Wg.inst[2].master.arc_list)
        print("a=", a6)
        # Wg.inst[-2]
        ...
        a8 = compute_arc_length(Wg.inst[3].master.arc_list)
        print("a=", a8)
        self.extract_photonic_ports(
            inst=Wg.inst[list(Wg.inst)[-1]],
            port_names=['PORT_IN','PORT_OUT'],
    ...
    def place_heater_ring(self,ubend_span):
        # heater parameters
        heater_radius_offset = 1.5  # 0.5 #0.45 # distance from top bottom of peanut u-bend
        resistance = 200
        contact_dist = 2
        contact_width = 1  # 1.5
        ...
        heater_electrode_top_y_span = 3.25  # 1.2
        ...
        heater_electrode_bottom_y_span = 3.25
        # Advanced parameters
        heater_disk_layer_extension = 0.5
        ...
        if heater_electrode_bottom_y_span is None:
            ...
            heater_params['width'] = 2 # heater width 0.8
