    ...
    def place_gratings(self):
        if self.add_grating_left and self.add_grating_right:
            ...
            if self.grating_band_radius == 0:
                ...
                pad_params_left = deepcopy(adiabatic_band_params)
                pad_params_left['arc_params'] = [dict(arc_type="straight_wg", width=self.wg_width, length=pad1)]

                pad_params_right = deepcopy(adiabatic_band_params)
                pad_params_right['arc_params'] = [dict(arc_type="straight_wg", width=self.wg_width, length=pad2)]

                temp_wg_padr = self.new_template(params=pad_params_right, temp_cls=AdiabaticPaths)
                temp_wg_padl = self.new_template(params=pad_params_left, temp_cls=AdiabaticPaths)
                right_wg_inst = self.add_instance_port_to_port(inst_master=temp_wg_padr,
                                                               instance_port_name='PORT_IN',
                                                               self_port_name='PORT1')

                left_wg_inst = self.add_instance_port_to_port(inst_master=temp_wg_padl,
                                                              instance_port_name='PORT_IN',
                ...
                pad1 = pad - 15 - self.grating_band_radius * 1.8700958466
                pad2 = pad + 15 - self.grating_band_radius * 1.8700958466
                # adiabatic_band_params['arc_params'] = [
