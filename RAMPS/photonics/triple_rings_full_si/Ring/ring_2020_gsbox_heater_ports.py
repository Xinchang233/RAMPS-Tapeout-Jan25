...
from ...triple_rings_full_si.Spoke.SpokeBase import SpokeBase
from ...triple_rings_full_si.WaveguideBase.CosineWaveguide import CosineWaveguide

from typing import TYPE_CHECKING, List, Union, Optional
from ...triple_rings_full_si.ViaStack_test.ViaStack import ViaStack
from ...triple_rings_full_si.Importers.Gratings.higheff1300lm5um.GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A import GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A
...
from ...triple_rings_full_si.Ring.ringheater import RingHeater
...
class RingBase(BPG.PhotonicTemplateBase):
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        # Added by Djordje
        self.cos_amplitude_multiplier = self.params['cos_amplitude_multiplier']
        self.use_adiabatic_access_wg = self.params['use_adiabatic_access_wg']
        self.use_adiabatic_drop_wg = self.params['use_adiabatic_drop_wg']
        self.amplitude_fraction = None
        ...
        self.draw_gsbox = params['draw_gsbox']
    ...
    def get_default_param_values(cls):
        return dict(
            ...
            cos_amplitude_multiplier=0.5,
            use_adiabatic_access_wg=False,
            use_adiabatic_drop_wg=False,
        )
    ...
    def get_params_info(cls):  # Returns definition of the parameters.
        return dict(
            ...
            cos_amplitude_multiplier='Multiplier used to reduce or increase amplitude of the cosine couplers',
            use_adiabatic_access_wg='Boolean param that specifies if cosine or adiabatic access coupler is drawn',
            use_adiabatic_drop_wg='Boolean param that specifies if cosine or adiabatic drop coupler is drawn',
            ...
            draw_gsbox = 'boolean, draw box padding for gs connect - not for probing',
    ...
    def draw_layout(self):
        ...
        if self.coupling_slot and self.coupling_slot > 0:
            if self.use_adiabatic_access_wg:
                self.draw_adiabatic_access_waveguide(gap=self.coupling_slot)
            elif self.amplitude_fraction:
                self.draw_access_waveguide(
                    cosine_amplitude=self.amplitude_fraction * self.rout,
                ...
                self.draw_access_waveguide(
                    cosine_amplitude=self.cos_amplitude_multiplier * self.rout,
                    cosine_length=self.access_length,
                    wg_width=self.wg_width,
                    layer=self.layer
                )
    ...
    def create_silicon_ring(self):
        ...
        self.add_photonic_port(name='PORT_RING_BOTTOM', center=[0, -self.rout],
                               layer=['si_full_free', 'port'],
                               orient='R90', width=0.1, show=False)
        self.add_photonic_port(name='PORT_RING_TOP', center=[0, self.rout],
                               layer=['si_full_free', 'port'],
                               orient='R90', width=0.1, show=False)


    def draw_adiabatic_access_waveguide(self, gap, layer=('si_full_free', 'drawing')):


        init_port = self.add_photonic_port(name='PORT_IN',
                                           center=(14.24509735426525+2, -gap-1.075), orient='R0', width=0.4, layer=('si_full_free', 'port'))

        Wg = AdiabaticRouter(gen_cls=self, init_port=init_port, layer=layer, name='init_port')

        Wg.add_straight_wg(length=2)
        Wg.add_wrapped_ring_coupler_centport(rmin_in=10, alpha_zero_in=np.pi / 6,
                                             r_coupling=6 + 0.4 / 2 + 0.1,
                                             angle_coupling=np.pi / 5, w_coupling=0.4)

        length = get_arc_length(Wg.inst[1].master.arc_list)
        height_of_wrapped_coupler = get_arc_height(Wg.inst[1].master.arc_list)

        print(length, height_of_wrapped_coupler)
        ...
        self.add_photonic_port(name='PORT1',
                               center=(
                                   length+2,
                                   -gap - 1.075),
                               orient='R180', width=0.4, layer=('si_full_free', 'port'))
        self.add_photonic_port(name='PORT0',
                               center=(
                                   -length - 2,
                                   -gap - 1.075),
                               orient='R0', width=0.4, layer=('si_full_free', 'port'))
    ...
    def place_gs_boxesonly(self):
        ...
        in_layer = self.inner_electrode_ring_layers[-1] #outer_electrode_ring_layers
        out_layer = self.outer_electrode_ring_layers[-1] #outer_electrode_ring_layers
        ...
        #
        # Rect_inner = PhotonicRect(layer=in_layer, bbox=box)
        ...
        #
        # Rect_inner1 = PhotonicRect(layer=in_layer, bbox=box1)
        ...
        #             top=-self.rout / 4, resolution=self.grid.resolution, unit_mode=False)
        #
        # Rect_inner2 = PhotonicRect(layer=in_layer, bbox=box2)
        # self.add_obj(Rect_inner2)

        box2 = BBox(left=-electrode_rin_n - pad_size -1.5, bottom=-self.rout / 4,
                    right=-electrode_rin_n,
                    top=self.rout / 4, resolution=self.grid.resolution, unit_mode=False)

        Rect_inner2 = PhotonicRect(layer=in_layer, bbox=box2)
        self.add_obj(Rect_inner2)

        self.add_photonic_port(name='PORT_INNER_PIN', center=[box2.left, box2.top - (box2.top-box2.bottom)/2],layer=['RX', 'port'], orient='R0',width=(box2.top-box2.bottom),show=False)
        # (self,
        ...
        Rect_outer = PhotonicRect(layer=out_layer, bbox=box3)
        self.add_obj(Rect_outer)
        self.add_photonic_port(name='PORT_OUTER_PIN', center=[box3.right, box3.top - (box3.top - box3.bottom) / 2], layer=['RX', 'port'],
                               orient='R180', width=(box3.top - box3.bottom), show=False)



    def place_heater_ring(self):
        # heater parameters
        heater_radius_offset= 0.47
        resistance= 200
        contact_dist= 1
        contact_width= 1
        heater_device_layer = ('RX', 'drawing')
        heater_device_layer_RX = ('RX', 'drawing')
        # heater_disk_layers = ('nw1phot', 'drawing')
        heater_disk_layers = [('n_heavy_phot', 'drawing'), ('SBLK', 'drawing')]
        ...
        # Advanced parameters
        heater_disk_layer_extension= 0.05
        # Technology parameter
        heater_electrode_bottom_layer= ('si_full', 'drawing')

        heater_params = dict(
            rout=(self.rout - self.ring_width -
                  max(self.p_via_radius_offset, self.n_via_radius_offset) -
        ...
        if heater_electrode_bottom_y_span is None:
            ...
            self.add_instance(master=heater_master, orient='R0')
    ...
    def place_gs_electrodes(self):
        ...
        Rect_inner = PhotonicRect(layer=['M5', 'drawing'], bbox=box)
        self.add_obj(Rect_inner)
        ...
        Rect_inner1 = PhotonicRect(layer=['M5', 'drawing'], bbox=box1)
        self.add_obj(Rect_inner1)
        ...
        Rect_inner2 = PhotonicRect(layer=['M5', 'drawing'], bbox=box2)
        self.add_obj(Rect_inner2)
        ...
        Rect_outer = PhotonicRect(layer=['M5', 'drawing'], bbox=box3)
        self.add_obj(Rect_outer)
    ...
    def place_gratings(self):
        if self.add_grating_left and self.add_grating_right:
            ...
            adiabatic_band_params = dict(layer=self.layer, port_layer=['si_full', 'port'], radius_threshold=1.5,
