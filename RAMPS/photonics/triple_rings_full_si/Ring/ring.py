...
class RingBase(BPG.PhotonicTemplateBase):
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        self.grating_router_bool = self.params['grating_router_bool']
        self.grating_router_len = self.params['grating_router_len']
    ...
    def get_default_param_values(cls):
        return dict(
            ...
            grating_router_bool =False,
            grating_band_radius = 5,
            grating_router_len = 45
        )
    ...
    def get_params_info(cls):  # Returns definition of the parameters.
        return dict(
            ...
            grating_router_bool = 'Boolean determening whether additional routing is needed. Not the best implementation',
            grating_router_len = 'None'
        )
    ...
    def place_gratings(self):
        if self.add_grating_left and self.add_grating_right:
            ...
            if self.grating_band_radius == 0:
                ...
                # Add grating
                temp = self.new_template(params=None, temp_cls=GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A)
                if self.grating_router_bool:
                    Wg_router = AdiabaticRouter(gen_cls=self, init_port=inst_90_left['PORT_OUT'],layer=self.layer)
                    Wg_router.add_straight_wg(length=temp.bound_box.width - compute_arc_length(temp_90_left.arc_list) - 21.9)
                    Wg_router.add_bend_90(rmin = self.grating_band_radius)
                    Wg_router.add_straight_wg(length=self.grating_router_len)
                    Wg_router.add_bend_90(rmin=self.grating_band_radius)
                    right_inst = self.add_instance_port_to_port(inst_master=temp,
                                                                instance_port_name='PORT_OUT',
    ...
    def place_gratings2(self):
        str_section = 82
        Wg_left = AdiabaticRouter(gen_cls=self, init_port=self._photonic_ports['PORT1'],layer=self.layer)
        Wg_right = AdiabaticRouter(gen_cls=self, init_port=self._photonic_ports['PORT0'], layer=self.layer)
        Wg_left.add_straight_wg(length=str_section)
        Wg_right.add_straight_wg(length=str_section)

        temp = self.new_template(params=None, temp_cls=GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A)

        right_inst = self.add_instance_port_to_port(inst_master=temp,
                                                    instance_port_name='PORT_OUT',
                                                    self_port=Wg_right.port)
        left_inst = self.add_instance_port_to_port(inst_master=temp,
                                                   instance_port_name='PORT_OUT',
                                                   self_port=Wg_left.port)
