...
# layout/Importers/Gratings/higheff1300lm5um/GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A_nobend.py
...
class RingBase(BPG.PhotonicTemplateBase):
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        self.grating_turnaround_left = params['grating_turnaround_left']
        self.grating_turnaround_right = params['grating_turnaround_right']
        self.grating_afterturn_added_len_right = params['grating_afterturn_added_len_right']
        self.grating_afterturn_added_len_left = params['grating_afterturn_added_len_left']
        self.grating_class_right = params['grating_class_right']
        self.grating_class_left = params['grating_class_left']
    ...
    def get_default_param_values(cls):
        return dict(
            ...
            grating_turnaround_left = False,
            grating_turnaround_right = False,
            grating_afterturn_added_len = 0
        )
    ...
    def get_params_info(cls):  # Returns definition of the parameters.
        return dict(
            ...
            grating_turnaround_left = 'Boolean for chosing to turn grating opposite direction',
            grating_turnaround_right = 'Boolean for chosing to turn grating opposite driection',
            grating_afterturn_added_len_right = 'Value that if above 0 will be added as a waveguide before the grating',
            grating_afterturn_added_len_left = 'Value that if above 0 will be added as a waveguide before the grating',
            grating_class_right = 'class name for grating on right',
            grating_class_left = 'class name for grating on left'
        )
        ...
        if self.add_grating_left and self.add_grating_right:
            ...
            if self.grating_band_radius == 0:
                ...
                bend90_params['arc_params'] = [
                    dict(arc_type="90_bend", rmin=self.grating_band_radius, turn_left=False,
                         width=[self.wg_width, 0.5])]
                ...
                # self.grating_class_right = GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A
                # self.grating_class_left = GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A
                #GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A
                if self.grating_class_left == 1:
                    temp_grat_left = self.new_template(params=None, temp_cls=GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A_nobend)
                else:
                    temp_grat_left = self.new_template(params=None, temp_cls=GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A)
                if self.grating_class_right == 1:
                    temp_grat_right = self.new_template(params=None, temp_cls=GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A_nobend)
                else:
                    temp_grat_right = self.new_template(params=None, temp_cls=GcUnidirWl1300nmMfd5000nmApodDown_fromAL11A)
                ...
                adiabatic_band_params2 = dict(layer=self.layer, port_layer=['RX', 'port'], radius_threshold=1.5,
                                             curvature_rate_threshold=0.7, merge_arcs=False)
                adiabatic_band_params2['arc_params'] = [
                    dict(arc_type="straight_wg", width=self.wg_width,
                         length=self.grating_afterturn_added_len_left)]
                temp_wg2 = self.new_template(params=adiabatic_band_params2, temp_cls=AdiabaticPaths)
                temp_wg3_params = deepcopy(adiabatic_band_params2)
                temp_wg3_params['arc_params'] = [
                    dict(arc_type="straight_wg", width=self.wg_width,
                         length=self.grating_afterturn_added_len_right)]
                temp_wg3 = self.new_template(params=temp_wg3_params, temp_cls=AdiabaticPaths)
                ...
                right_wg_inst = self.add_instance_port_to_port(inst_master=temp_wg,
                                                               ...
                                                               self_port_name='PORT1')
                #LEFT
                if self.grating_turnaround_left:
                    temp_90_left2 = self.new_template(params=bend90_params, temp_cls=AdiabaticPaths)
                    # regular bend
                    inst_90_left = self.add_instance_port_to_port(inst_master=temp_90_left2,
                                                                  instance_port_name='PORT_IN',
                                                                  self_port=left_wg_inst['PORT_OUT'],
                    ...
                    inst_90_left = self.add_instance_port_to_port(inst_master=temp_90_left,
                                                                  ...
                                                                  self_port=left_wg_inst['PORT_OUT'],
                                                                  reflect=self.reflect)
                # added length to extend
                if self.grating_afterturn_added_len_left > 0:
                    left_wg_inst_end = self.add_instance_port_to_port(inst_master=temp_wg2,
                                                                   instance_port_name='PORT_IN',
                                                                   self_port=inst_90_left['PORT_OUT'])
                else:
                    left_wg_inst_end = inst_90_left

                #if self.grating_usev2_right:

                # add bend
                bend90_params_away = deepcopy(bend90_params)
                temp_awaybend = self.new_template(params=bend90_params_away, temp_cls=AdiabaticPaths)
                # regular bend
                inst_awaybend = self.add_instance_port_to_port(inst_master=temp_awaybend,
                                                               instance_port_name='PORT_IN',
                                                               self_port=left_wg_inst_end['PORT_OUT'],
                                                               reflect=self.reflect)

                #add wg
                away_wg_params = deepcopy(adiabatic_band_params2)
                away_wg_params['arc_params'] = [
                    dict(arc_type="straight_wg", width=self.wg_width,
                         length=95)]
                temp_away_wg = self.new_template(params=away_wg_params, temp_cls=AdiabaticPaths)
                inst_away_wg = self.add_instance_port_to_port(inst_master=temp_away_wg,
                                                               instance_port_name='PORT_IN',
                                                               self_port=inst_awaybend['PORT_OUT'],
                                                               reflect=self.reflect)
                # add bend
                bend90_params_away2 = deepcopy(bend90_params_left)
                temp_awaybend2 = self.new_template(params=bend90_params_away2, temp_cls=AdiabaticPaths)
                # regular bend
                inst_awaybend2 = self.add_instance_port_to_port(inst_master=temp_awaybend2,
                                                               instance_port_name='PORT_IN',
                                                               self_port=inst_away_wg['PORT_OUT'],
                                                               reflect=self.reflect)

                # insert grating
                left_inst = self.add_instance_port_to_port(inst_master=temp_grat_left,
                                                           instance_port_name='PORT_OUT',
                                                           self_port=inst_awaybend2['PORT_OUT'])

                #RIGHT
                if self.grating_turnaround_right:
                    temp_90_right2 = self.new_template(params=bend90_params_left, temp_cls=AdiabaticPaths)
                    # regular bend
                    inst_90_right = self.add_instance_port_to_port(inst_master=temp_90_right2,
                                                                  instance_port_name='PORT_IN',
                                                                  self_port=right_wg_inst['PORT_OUT'],
                                                                  reflect=self.reflect)
                else:
                    ...
                    inst_90_right = self.add_instance_port_to_port(inst_master=temp_90_right,
                                                                  ...
                                                                  reflect=self.reflect)
                # added length to extend
                if self.grating_afterturn_added_len_right > 0:
                    right_wg_inst_end = self.add_instance_port_to_port(inst_master=temp_wg3,
                                                                      instance_port_name='PORT_IN',
                                                                      self_port=inst_90_right['PORT_OUT'])
                else:
                    right_wg_inst_end = inst_90_right

                # insert grating
                right_inst = self.add_instance_port_to_port(inst_master=temp_grat_right,
                                                           instance_port_name='PORT_OUT',
                                                           self_port=right_wg_inst_end['PORT_OUT'])
