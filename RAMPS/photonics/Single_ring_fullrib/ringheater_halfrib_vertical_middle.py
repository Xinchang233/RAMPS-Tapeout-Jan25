...
class RingHeater_middle(BPG.PhotonicTemplateBase):
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        if self.params['electrode_bottom_x_span'] is None:
            ...
            self.electrode_bottom_x_span = 1.6
        if self.params['electrode_bottom_y_span'] is None:
            ...
            self.electrode_bottom_y_span = 6

    @classmethod
    def get_default_param_values(cls):
        return dict(
            electrode_top_x_span=None,
            electrode_top_y_span=None,
    ...
    def draw_heater_ring_and_contacts(self,
        ...
        #                  ]
        contact_points = [(-contact_dist / 2.0-self.electrode_bottom_x_span, -heater_rin+0.28),
                         (-contact_dist / 2.0, -heater_rin+0.28),
                         (-contact_dist / 2.0, contact_width / 2.0),
        ...
        contact_points = [(-contact_dist / 2.0 - self.electrode_bottom_x_span-0.4, -heater_rin + 0.65),
                          (-contact_dist / 2.0, -heater_rin + 0.65),
                          (-contact_dist / 2.0, -heater_rin + 0.8),
                          (-contact_dist / 2.0 - self.electrode_bottom_x_span-0.4,-heater_rin + 0.8)
                          ]
        polygon = PhotonicPolygon(
            resolution=self.grid.resolution,
            layer=device_layer,
