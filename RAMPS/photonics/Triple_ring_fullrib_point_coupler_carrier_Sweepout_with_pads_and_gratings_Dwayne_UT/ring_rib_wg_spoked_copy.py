...
from ..layout.Importers.Gratings.uni_grating import GcBidirWl1300nmMfd9200nmCurved
from importlib import import_module
...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def __init__(self, temp_db, lib_name, params, used_names, **kwargs):
        ...
        self.doping_spoke_info3 = self.params['doping_spoke_info3']


        #adding new carrier sweepout
        self.middle_inner_electrode_info=self.params['middle_inner_electrode_info']
        self.middle_outer_electrode_info = self.params['middle_outer_electrode_info']
        self.middle_via_info=self.params['middle_via_info']
        self.middle_doping_info=self.params['middle_doping_info']
    ...
    def get_params_info(cls) -> dict:
        return dict(
            # Parameters of rib-waveguide ring
            # --------------------------------
            grating_params='Params specifying the grating',
            ...
            taper_layer2='xcv',
            coup_gap_drop='sds',
            doping_spoke_info3='asf',
            doping_spoke_info2='asf',
            doping_spoke_info1='ad',

            middle_inner_electrode_info='dds',
            middle_outer_electrode_info = 'sdsd',
            middle_via_info = 'sd',
            middle_doping_info = 's',
    ...
    def get_default_param_values(cls) -> dict:
        ...
        return dict(
            ...
            taper_layer2=None,
            coup_gap_drop=None,
            doping_spoke_info3=None,
            doping_spoke_info2=None,
            doping_spoke_info1=None,
            middle_inner_electrode_info=None,
            middle_outer_electrode_info = None,
            middle_via_info = None,
            middle_doping_info = None,
        )
    ...
    def draw_layout(self):
        ...
        self.draw_doping_spokes3(self.doping_spoke_info3)

        #adding new carrier sweepout region
        self.middle_draw_concentric_rings(self.middle_doping_info)
        self.middle_draw_concentric_rings(self.middle_inner_electrode_info)
        self.middle_draw_concentric_rings(self.middle_outer_electrode_info)
        self.middle_place_circular_via_stack()

        delta_phi = np.pi * self.middle_inner_electrode_info[0]['angle0'] / 180

        a=self.r_core_cent
        b=(self.middle_inner_electrode_info[-1]['dist0']+self.middle_inner_electrode_info[-1]['dist1'])/2

        ### Midle ring right M6
        self.add_rect(layer=('BA','drawing'),
                      bbox=BBox(right=(a+b)* np.cos(delta_phi)+0.5,
                                bottom=-(a+b)* np.sin(delta_phi)-0.25,
                                left=(a+b)* np.cos(delta_phi)-0.5,
                                top=(a+b)* np.sin(delta_phi)+0.25,
                                resolution=self.grid.resolution)
                      )

        ### Midle ring centre M6 pad
        self.add_rect(layer=('BA', 'drawing'),
                      bbox=BBox(right=2,
                                bottom=(a + b) -0.25,
                                left=-2,
        ...
        wire_width = 4
          ## midle ring M6 routing to uper right
            ## horizontal part
        self.add_rect(layer=('BA', 'drawing'),
                      bbox=BBox(right=1.5*self.gs_pad_pitch+0.5*wire_width,
                                bottom=(a + b)  + 0.25 +7,
                                left=-2,
                                top=(a + b)  + 0.25 +7+wire_width,
                                resolution=self.grid.resolution)
                    )
            ## vertical part
        dim1 = 11.7
        self.add_rect(layer=('BA', 'drawing'),
                      bbox=BBox(right=1.5*self.gs_pad_pitch+0.5*wire_width,
                                bottom=(a + b)  + 0.25 +7,
                                left=1.5*self.gs_pad_pitch-0.5*wire_width,
                                top=self.gs_pad_to_ring_dist+0.5*dim1,
                                resolution=self.grid.resolution)
                    )
        ...
        delta_phi = np.pi * self.middle_outer_electrode_info[0]['angle0'] / 180

        a = self.r_core_cent
        b = (self.middle_outer_electrode_info[-1]['dist0'] + self.middle_outer_electrode_info[-1]['dist1']) / 2
        ...
        self.add_rect(layer=('M5', 'drawing'),
            ...
            ## midle ring center to Pad M5 M6 M7 routing
        Midle_M5_top  = -(a + b) - 0.25-2
        Midle_M5_bottom = -((a + b)+0.25+4)
              # part one: before 5->6->vias
        M7_width = 4
        M6_width = 4
        via_stack_master_56 = self.new_template(params=dict(top_layer=self.inner_electrode_info1[-1]['layer'],
                                                         bottom_layer=self.outer_electrode_info1[-1]['layer'],
                                                         top_x_span=M6_width,
                                                         top_y_span=1.5*self.gs_bottom_length,
        ...
        via_stack_master_67 = self.new_template(params=dict(top_layer=('UA','drawing'),
                                                         bottom_layer=self.inner_electrode_info1[-1]['layer'],
                                                         top_x_span=M7_width,
                                                         top_y_span=2*self.gs_bottom_length,
                                                         bottom_x_span=M6_width,
                                                         bottom_y_span=1.5*self.gs_bottom_length,
                                                         align='center_align',
                                                         top_bot_offset=0.0),
                                             temp_cls=ViaStack)
             ## add vias
        K =4 
        self.add_instance(master=via_stack_master_56,
                          inst_name='via_M5_M6_midle_ring',
                          loc=(K*self.r_core_cent-0.5*self.gs_bottom_length,0.5*(Midle_M5_top+Midle_M5_bottom)),
                          orient='R90')
        self.add_instance(master=via_stack_master_67,
                          inst_name='via_M6_M7_midle_ring',
                          loc=(K*self.r_core_cent-0.5*self.gs_bottom_length,0.5*(Midle_M5_top+Midle_M5_bottom)),
                          orient='R90')
             ##
        self.add_rect(layer=('M5', 'drawing'),
                      bbox=BBox(right=K*self.r_core_cent,
                                bottom=Midle_M5_bottom,
                                left=-1,
                                top=Midle_M5_top ,
                                resolution=self.grid.resolution)
                      )
             ## M7 horiontal M7 routing
        self.add_rect(layer=('UA', 'drawing'),
                      bbox=BBox(right=2.5*self.gs_pad_pitch+0.5*wire_width,
                                bottom=0.5*(Midle_M5_bottom+Midle_M5_top)-0.5*M7_width,
                                left=K*self.r_core_cent-self.gs_bottom_length,
                                top=0.5*(Midle_M5_bottom+Midle_M5_top)+0.5*M7_width ,
                                resolution=self.grid.resolution)
                      )
             ## M7 vertical M7 routing
        dim1 = 11.7
        self.add_rect(layer=('UA', 'drawing'),
                      bbox=BBox(right=2.5*self.gs_pad_pitch+0.5*M7_width,
                                bottom=Midle_M5_bottom,
                                left=2.5*self.gs_pad_pitch-0.5*M7_width,
                                top=self.gs_pad_to_ring_dist+dim1/2,
                                resolution=self.grid.resolution)
                      )


        #add metal contact layers for heater and sgs pads


    def draw_doping_spokes3(self,doping_spoke_info):
        """Draw all the doping spokes"""
        if self.doping_spoke_info3:
            for i, doping_spoke_info in enumerate(self.doping_spoke_info3):
    ...
    def place_contact_electrodes_basic(self):
        ...
        self.add_rect(layer=self.outer_electrode_info1[-1]['layer'],
            ...
            ### left ring M5 routing to Left bottom pad
         # horizontal part
        M5_width = 2
        M6_width = 4
        
        
        via_stack_master_56_LR = self.new_template(params=dict(top_layer=self.inner_electrode_info1[-1]['layer'],
                                                         bottom_layer=self.outer_electrode_info1[-1]['layer'],
                                                         top_x_span=M6_width,
                                                         top_y_span=1.5*self.gs_bottom_length,
                                                         bottom_x_span=M5_width,
                                                         bottom_y_span=self.gs_bottom_length,
                                                         align='center_align',
                                                         top_bot_offset=0.0),
                                             temp_cls=ViaStack)
        self.add_instance(master=via_stack_master_56_LR,
                          inst_name='via_M5_M6_LEFT_ring',
                          loc=(wire1_left+0.5*self.gs_bottom_length,0.5*(wire1_bottom+wire1_top)),
                          orient='R90')
        
        self.add_rect(layer=self.inner_electrode_info1[-1]['layer'],
                      bbox=BBox(right=wire1_left+self.gs_bottom_length, # checked
                                bottom=0.5*(wire1_bottom+wire1_top)-0.5*M6_width, # checked
                                left=-self.gs_pad_pitch-0.5*M5_width,    # checked
                                top=0.5*(wire1_bottom+wire1_top)+0.5*M6_width,       # checked
                                resolution=self.grid.resolution)
                      )
         # vertical part
        self.add_rect(layer=self.inner_electrode_info1[-1]['layer'],
                      bbox=BBox(right=-self.gs_pad_pitch+0.5*M6_width, # checked
                                bottom=wire1_bottom-self.gs_pad_to_ring_dist-0.5*dim1, # checked
                                left=-self.gs_pad_pitch-0.5*M6_width,    # checked
                                top=0.5*(wire1_top+wire1_bottom)+0.5*M6_width,       # checked
                                resolution=self.grid.resolution)
                      )
        ...
        ### right ring M5 routing to Right bottom pad
        ...
        via_stack_master_56_LR = self.new_template(params=dict(top_layer=self.inner_electrode_info1[-1]['layer'],
                                             ...
                                             temp_cls=ViaStack)
        self.add_instance(master=via_stack_master_56_LR,
                          inst_name='via_M5_M6_Right_ring',
                          loc=(wire1_right-0.5*self.gs_bottom_length,0.5*(wire1_bottom+wire1_top)),
                          orient='R90')
          #  # horizontal part
        self.add_rect(layer=self.inner_electrode_info1[-1]['layer'],
                      bbox=BBox(right=self.gs_pad_pitch+0.5*M6_width, # checked
                                bottom=0.5*(wire1_bottom+wire1_top)-0.5*M6_width, # checked
                                left=wire1_right-self.gs_bottom_length,#-self.gs_pad_pitch-0.5*M5_width,    # checked
                                top=0.5*(wire1_bottom+wire1_top)+0.5*M6_width,       # checked
                                resolution=self.grid.resolution)
                      )
         # vertical part
        ...
        #### vertical wire upto middle pad M6 routing
        self.midle_wire_top = wire2_top
        
        wire_width = 4
        self.midle_wire_width = wire_width
        self.add_rect(layer=self.inner_electrode_info1[-1]['layer'],
                      bbox=BBox(right=self.midle_wire_width/2,
                                bottom=-self.gs_pad_to_ring_dist-0.5*dim1, ### this has not been solved
                                left=-self.midle_wire_width/2,
                                top=self.midle_wire_top,
                                resolution=self.grid.resolution)
                      )
    ...
    def place_heater_contact_electrodes_basic(self):
        ...
        # M7 routing left heater signal
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_top-metal_width,
                                left=-2.5*self.gs_pad_pitch,
                                top=wire1_top,
                                resolution=self.grid.resolution)
                      )
        
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=-2.5*self.gs_pad_pitch+0.5*metal_width,
                                bottom=wire1_top-metal_width,
                                left=-2.5*self.gs_pad_pitch-0.5*metal_width,
                                top=self.gs_pad_to_ring_dist+0.5*dim1,
                                resolution=self.grid.resolution)
                      )
        
        # heater wiring to the left upper pad
        # # ## M7 pad heater pad left 2
        wire1_top = 18 * offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span/2-0.55
        ...
        self.add_rect(layer=self.heater_electrode_top_layer,
         ...
         # via buff box
        buff_wdith = 5    # greater than 4.86
        M7_width = 8
        via_y_span = M7_width
        via_x_span = buff_wdith
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_left+buff_wdith,
                                bottom=wire1_top - via_y_span,
                                left=wire1_left,  # good
                                top=wire1_top, # good
                                resolution=self.grid.resolution)
                      )
        via_stack_master_67 = self.new_template(params=dict(top_layer=('UB', 'drawing'),
                                                         bottom_layer=self.heater_electrode_top_layer,  # M7
                                                         top_x_span=via_x_span,
                                                         top_y_span=via_y_span,
                                                         bottom_x_span=via_x_span,
                                                         bottom_y_span=via_y_span,
                                                         align='center_align',
                                                         top_bot_offset=0.0),
                                             temp_cls=ViaStack)

        self.add_instance(master=via_stack_master_67,
                          inst_name='via_M6_M7_heater',
                          loc=(wire1_left+0.5*buff_wdith,wire1_top-via_y_span/2),
                          orient='R0')
        
        ## M7 routing. heater ground
        self.add_rect(layer=('UB','drawing'),
                      bbox=BBox(right=0.5*self.gs_pad_pitch,
                                bottom=wire1_top - via_y_span,
                                left=wire1_left,
                                top=wire1_top,
                                resolution=self.grid.resolution)
        ...
        self.add_rect(layer=('UB','drawing'),
                      bbox=BBox(right=0.5*self.gs_pad_pitch+0.5*via_y_span,
                                bottom=wire1_top - via_y_span,
                                left=0.5*self.gs_pad_pitch-0.5*via_y_span,
                                top=self.gs_pad_to_ring_dist+0.5*dim1,
                                resolution=self.grid.resolution)
        )

        # heater wiring to the left upper pad
        # # ## M7 pad heater pad center 1
        wire1_top = 18 * offset_distance
        wire1_bottom = -self.heater_electrode_top_y_span/2-0.55
        ...
        # M7 routing center heater signal
            # add length
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_top,
                                left=wire1_left,
                                top=wire1_top+2*metal_width,
                                resolution=self.grid.resolution)
                      )

        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_top+2*metal_width,
                                left=-1.5*self.gs_pad_pitch,
                                top=wire1_top+3*metal_width,
                                resolution=self.grid.resolution)
                      )
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=-1.5*self.gs_pad_pitch+0.5*metal_width,
                                bottom=wire1_top+2*metal_width,
                                left=-1.5*self.gs_pad_pitch-0.5*metal_width,
                                top=self.gs_pad_to_ring_dist+0.5*dim1,
                                resolution=self.grid.resolution)
                      )
        ...
        # M7 routing right heater signal
            # add length
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_top,
                                left=wire1_left,
                                top=wire1_top+5*metal_width,
                                resolution=self.grid.resolution)
                      )

        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_top+5*metal_width,
                                left=-0.5*self.gs_pad_pitch,
                                top=wire1_top+6*metal_width,
                                resolution=self.grid.resolution)
                      )
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=-0.5*self.gs_pad_pitch+0.5*metal_width,
                                bottom=wire1_top+5*metal_width,
                                left=-0.5*self.gs_pad_pitch-0.5*metal_width,
                                top=self.gs_pad_to_ring_dist+0.5*dim1,
                                resolution=self.grid.resolution)
                      )
    ...
    # add optical routing and gratings
        router = AdiabaticRouter(gen_cls=self, init_port=self._photonic_ports['PORT1'],
                                 layer=('si_full_free', 'drawing'), name='r1')
        router.add_bend_90(rmin=25, turn_left=False, width=0.41)
        router.add_straight_wg(length=5)
        router.add_bend_90(rmin=25, turn_left=True)

        router.add_straight_wg(length=150,width=0.35)


        x_cor_of_final_wg = router.inst[3].bound_box.right
        y_cor_of_final_wg = router.inst[3].bound_box.bottom
        P3 = self.add_photonic_port(name='bend0', orient='R180',
                                    center=(x_cor_of_final_wg, y_cor_of_final_wg - 0.41 / 2), width=0.41,
                                    layer=('RX', 'port'), resolution=self.grid.resolution, unit_mode=False, show=False)

        self.add_instance(
            master=self.gc_master,
            loc= (x_cor_of_final_wg, y_cor_of_final_wg+0.41/2),
            # Offset site by location of input port, to put input port at desired location
            orient='R180',
        )


        router1 = AdiabaticRouter(gen_cls=self, init_port=self._photonic_ports['PORT0'],
                                 layer=('si_full_free', 'drawing'), name='r1')
        router1.add_bend_90(rmin=25, turn_left=True, width=0.41)
        router1.add_straight_wg(length=5)
        router1.add_bend_90(rmin=25, turn_left=False)
        router1.add_straight_wg(length=150,width=0.35)
        x_cor_of_final_wg = router1.inst[3].bound_box.left
        y_cor_of_final_wg = router1.inst[3].bound_box.bottom
        P3 = self.add_photonic_port(name='bend1', orient='R0',
                                    center=(x_cor_of_final_wg, y_cor_of_final_wg - 0.41 / 2), width=0.41,
                                    layer=('RX', 'port'), resolution=self.grid.resolution, unit_mode=False, show=False)
    ...
    def middle_draw_concentric_rings(self, ring_info):
        """
        Draws concentric rings corresponding to doping layers, silicon contact rails, salicide layers, ring electrodes
        """
        for ring_count in range(len(ring_info)):
    ...
    def middle_place_circular_via_stack(self):
        """
        Draws azimuthally placed via stacks
        """
        for count in range(len(self.middle_via_info)):
            # calculate angular step from azimuthal distance
            angle_step = (180 / np.pi) * self.middle_via_info[count]['azim_dist'] / \
                         (self.r_core_cent + self.middle_via_info[count]['dist'])

            # calculate the number of vias
            via_num = int((self.middle_via_info[count]['angle1'] - self.middle_via_info[count]['angle0']) // angle_step)

            # instantiate via stacks
            for via_count in range(via_num):
                delta_phi = np.pi * angle_step / 180
                phi = np.pi * self.middle_via_info[count]['angle0'] / 180 + via_count * delta_phi
                self.add_via_stack(
                    bot_layer=self.middle_via_info[count]['layer_bottom'],
                    top_layer=self.middle_via_info[count]['layer_top'],
                    loc=(self.ring_loc[0] + (self.r_core_cent + self.middle_via_info[count]['dist']) * np.cos(phi),
                         self.ring_loc[1] + (self.r_core_cent + self.middle_via_info[count]['dist']) * np.sin(phi))
                )
    ...
    def place_gs_electrodes(self):
        ...
        self.add_rect(layer=('PROBE','drawing'),
                      bbox=BBox(left=self.left_pad_loc[0] - self.gs_pad_width / 2,
                                bottom=self.left_pad_loc[1] - self.gs_pad_length - dim1,
                                right=self.left_pad_loc[0] + self.gs_pad_width / 2,
                                top=self.left_pad_loc[1],
...
if __name__ == '__main__':
    spec_file = 'Triple_ring_fullrib_point_coupler_carrier_Sweepout_with_pads_and_gratings_Dwayne_UT/specs/full_rib_vertical_specs.yaml'
