    ...
    def draw_layout(self):
        ...
        for bot_metal_name, _ in metal_pairs:
            ...
            if self.side_align_x and self.side_align_y:
                ...
                layer_rect = self.add_rect(
                    layer=(bot_metal_name, 'drawing'),
                    bbox=BBox(
                        bottom=top_y_span / 2 - y_span / 2,
                        top=top_y_span / 2 + y_span / 2,
                        left=0,
                        right=x_span,
                        resolution=self.grid.resolution,
                        unit_mode=False,
                ...
                layer_rect = self.add_rect(
                    ...
                    bbox=BBox(
                        bottom=0,
                        top=y_span,
                        left=top_x_span / 2 - x_span / 2,
                        right=top_x_span / 2 + x_span / 2,
                        resolution=self.grid.resolution,
                        unit_mode=False,
                    ),
                    unit_mode=False
        ...
        for ind in range(len(self.enclosure_list) - 1):
            ...
            if self.side_align_x and self.side_align_y:
                ...
                bbox = BBox(
                    bottom=offset_y,
                    top=y_span_enclosure - offset_y,
                    left=offset_x,
                    right=x_span_enclosure - offset_x,
                    resolution=self.grid.resolution,
                    unit_mode=False,
                )
            elif self.side_align_x and not self.side_align_y:
