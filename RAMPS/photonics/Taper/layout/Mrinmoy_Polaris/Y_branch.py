...
class Y_branch(BPG.PhotonicTemplateBase):
    ...
    def draw_layout(self) -> None:
        ...
        ##new_templete consists [params, tem_class, debug..]
        self.input_wg_temp = self.new_template(params=in_wg_params, temp_cls=AdiabaticPaths)
        ##add_instance consists [inst_name(i.e: templete's name), loc, orient, angle, reflect, nx, ny, spx: dim_type, spy, unit_mode]
        self.input_wg_inst = self.add_instance(self.input_wg_temp, loc=(0, 0), orient='R0', reflect=False)
        ##extract_photonic_ports [instance, port_names,port_renaming(opt), show]
        ...
        #add_instance_port_to_port consists [inst_master, instance_port_name,self_port, self_port_name, instance_name, reflect]
...
### num_spokes=0, so NO spokes -> spokes is like a grating on the inner side of arm of y-branch
        ...
        if self.add_triangles_boolean: #this adds the sparp pointy polygon in the layout
            self.add_triangles()

        if self.add_offset: #this adds the adiabatic path 12 i.e.: the extend WGs of the Y-branch
...
##spokes not being used since num_spokes=0
    ...
    def add_offset_bend(self):
        ...
        #inst1 and inst2 creates the top and bottom arm of the y_branch
