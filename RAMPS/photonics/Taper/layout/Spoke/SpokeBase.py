...
class SpokeBase(BPG.PhotonicTemplateBase):
    ...
    def draw_layout(self):
        ...
        for i in range(self.num):
            ...
            theta_min = theta0 - 0.25 * delta_theta * self.spk_wdth_pct
            theta_max = theta0 + 0.25 * delta_theta * self.spk_wdth_pct

            self.add_polygon(
                layer=self.layer,
                points=[
                    (self.rin * math.cos(theta_min), self.rin * math.sin(theta_min)),
                    (self.rout * math.cos(theta_min), self.rout * math.sin(theta_min)),
                    (self.rout * math.cos(theta_max), self.rout * math.sin(theta_max)),
                    (self.rin * math.cos(theta_max), self.rin * math.sin(theta_max)),
                ],
