...
class sourceandfilters2(BPG.PhotonicTemplateBase):
    ...
    def create_taper(self) -> None:
        taperparam = dict(port_layer=self.source_params['arb_ring_params']['wg_in_params']['port_layer'], width0=self.asefilter_params['ase_filter_params']['arb_ring_params']['wg_out_params']['width'], width1=self.source_params['arb_ring_params']['wg_in_params']['width'],
                      length=self.taper_length, layer=self.source_params['arb_ring_params']['wg_in_params']['layer'])
        self.taper_master = self.new_template(params=taperparam, temp_cls=LinearTaper)

    def create_source(self) -> None:
        self.filters_master['source'] = self.new_template(params=self.source_params['arb_ring_params'],
    ...
    def place_source(self) -> None:
        ...
        self.taper['out'] = self.add_instance_port_to_port(inst_master=self.taper_master,
                                                                ...
                                                                self_port=self.filters['source']['PORT_THROUGH'],
                                                                reflect=False)
