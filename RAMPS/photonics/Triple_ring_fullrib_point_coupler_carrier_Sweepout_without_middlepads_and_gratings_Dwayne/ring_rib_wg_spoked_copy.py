...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def place_heater_contact_electrodes_basic(self):
        ...
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=-1.5 * self.gs_pad_pitch + 0.5 * metal_width,
                                ...
                                top=self.heater_pad_to_ring_dist + 0.5 * dim1,
                                resolution=self.grid.resolution)
                      )
        ...
        self.add_rect(layer=('UB', 'drawing'),
                      bbox=BBox(right=1.5 * self.gs_pad_pitch,
                                bottom=wire1_top - via_y_span,
                                left=wire1_left,
                                top=wire1_top,
                                resolution=self.grid.resolution)
        ...
        self.add_rect(layer=('UB', 'drawing'),
                      bbox=BBox(right=1.5 * self.gs_pad_pitch + 0.5 * via_y_span,
                                bottom=wire1_top - via_y_span,
                                left=1.5 * self.gs_pad_pitch - 0.5 * via_y_span,
                                top=self.heater_pad_to_ring_dist + 0.5 * dim1,
                                resolution=self.grid.resolution)
                      )
    ...
    # add optical routing and gratings
        ...
        router.add_straight_wg(length=10)
        router.add_bend_90(rmin=25, turn_left=True)

        router.add_straight_wg(length=250,width=0.35)
        ...
        router1.add_straight_wg(length=10)
        router1.add_bend_90(rmin=25, turn_left=False)
        router1.add_straight_wg(length=250,width=0.35)
...
if __name__ == '__main__':
    spec_file = 'Triple_ring_fullrib_point_coupler_carrier_Sweepout_without_middlepads_and_gratings_Dwayne/specs/full_rib_vertical_specs.yaml'
