...
from ..triple_rings_full_si.Ring_new.wrapped_coupler_tapeout_full_rib_NEW import adiabatic_coupler_cena
...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def draw_KG_pieces(self):
        ...
        self.add_rect(layer=('KG', 'drawing'),
                      bbox=BBox(right=-6.1, bottom=6, left=-7, top=7.98, resolution=self.grid.resolution))
        self.add_rect(layer=('KG', 'drawing'),
                      bbox=BBox(right=-6.1, bottom=-7.98, left=-7, top=-6, resolution=self.grid.resolution))
    ...
    def place_contact_electrodes_basic(self):
        ...
        wire1_top = self.ring_loc[1]
        wire1_bottom = wire1_top - self.gs_bottom_width
        wire1_left = self.r_core_cent-self.inner_electrode_info[-1]['dist0']-2.4
        wire1_right = 14
        self.add_rect(layer=self.inner_electrode_info[-1]['layer'],
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
    ...
    def place_heater_contact_electrodes_basic(self):
        ...
        wire1_left = -self.contact_dist / 2-metal_width/2
        ...
        wire1_right = self.contact_dist / 2 + metal_width / 2
        self.add_rect(layer=self.heater_electrode_top_layer,
                      bbox=BBox(right=wire1_right,
                                bottom=wire1_bottom,
                                left=wire1_left,
...
if __name__ == '__main__':
    spec_file = 'Single_ring_fullrib_wrapped_coupler/specs/full_rib_vertical_specs.yaml'
    #spec_file = 'Single_ring_fullrib/specs/full_rib_horizontal_specs.yaml'
