...
class RingHeater(BPG.PhotonicTemplateBase):
    ...
    def draw_heater_ring_and_contacts(self,
        ...
        #                  ]
        top_contact=heater_rout*np.sin(40*np.pi/180)
        length_contact= heater_rout*np.cos(40*np.pi/180)

        length_rx_piece=heater_rin*np.cos(40*np.pi/180)

        # RX addition pieaces
        contact_points = [(-length_rx_piece, top_contact ),
                          (-length_rx_piece+3, top_contact ),
                          (-length_rx_piece+3, top_contact+2),
                          (-length_rx_piece, top_contact+2)
                          ]
        ...
        contact_points = [(-length_rx_piece, -top_contact - 2),
                            (-length_rx_piece+3, -top_contact-2),
                          (-length_rx_piece + 3, -top_contact),
                          (-length_rx_piece , -top_contact),
        ...
        contact_points = [(length_rx_piece-3, top_contact),
                          (length_rx_piece , top_contact),
                          (length_rx_piece , top_contact + 2),
                          (length_rx_piece-3, top_contact + 2)
                          ]
        ...
        contact_points = [(length_rx_piece - 3, -top_contact - 2),
                          (length_rx_piece , -top_contact-2),
                          (length_rx_piece, -top_contact),
                          (length_rx_piece-3, -top_contact ),
        ...
        # upper left
        contact_points = [(-length_contact, top_contact-contact_width),
                         (-contact_dist / 2.0, top_contact-contact_width),
                         (-contact_dist / 2.0,top_contact),
                         (-length_contact, top_contact)
                          ]
        polygon = PhotonicPolygon(
            resolution=self.grid.resolution,
            layer=device_layer,
        ...
        # lower left
        contact_points = [(-length_contact, -top_contact ),
                          (-contact_dist / 2.0, -top_contact ),
                          (-contact_dist / 2.0, -top_contact+ contact_width),
                          (-length_contact, -top_contact+ contact_width)
                          ]
        polygon = PhotonicPolygon(
            resolution=self.grid.resolution,
            layer=device_layer,
        ...
        # BA
        contact_points = [(contact_dist / 2.0+self.electrode_top_x_span, top_contact ),
                          (contact_dist / 2.0, top_contact ),
                          (contact_dist / 2.0, -top_contact),
                          (contact_dist / 2.0+self.electrode_top_x_span, -top_contact)
                          ]
        polygon = PhotonicPolygon(
            resolution=self.grid.resolution,
            layer=('BA','drawing'),
        ...
        # M5
        contact_points = [(-contact_dist / 2.0, -top_contact),
                          (-contact_dist / 2.0, top_contact),
                          (-contact_dist / 2.0-self.electrode_top_x_span, top_contact),
                          (-contact_dist / 2.0-self.electrode_top_x_span, -top_contact )
                          ]
        ...
        if disk_layers:
            ...
            left_heater_loc = (-self.contact_dist / 2.0, top_contact-self.electrode_top_y_span/2)
            self.add_instance(
            ...
            right_heater_loc = (self.contact_dist / 2.0, top_contact-self.electrode_top_y_span/2)
            self.add_instance(
                master=electrode_master,
            ...
            right_electrode = electrode_top_rect.transform(loc=right_heater_loc, orient='R0', unit_mode=False,
                                                           copy=True)
            self.add_label(
