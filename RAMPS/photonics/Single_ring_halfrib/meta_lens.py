...
class RingRibWg(BPG.PhotonicTemplateBase):
    ...
    def modphoton_addition(self):
        ...
        self.add_rect(layer=('si_full_free', 'drawing'),
                      bbox=BBox(right=20,
                                bottom=-6.26,
                                left=-20,
                                top=6.26,
        ...
        y=0.32
        gap=0.12
        self.add_rect(layer=('si_etch', 'drawing'),
                      bbox=BBox(right=x,
                                bottom=0,
                                left=0,
                                top=y,
        ...
        self.add_rect(layer=('si_etch', 'drawing'),
                      bbox=BBox(right=3 * x + 3 * gap+0.09,
                                bottom=0,
                                left=3 * x + 3 * gap,
                                top=1.15,
                                resolution=self.grid.resolution)
        ...
        self.add_rect(layer=('si_etch', 'drawing'),
                      bbox=BBox(right=3 * x + 4 * gap + 0.09 + 0.09,
                                bottom=0,
                                left=3 * x + 4 * gap + 0.09,
                                top=1.15, #0.84 for 0.12 um
                                resolution=self.grid.resolution)
                      )
        ed=3 * x + 4 * gap + 0.09 + 0.09
        d=0.37
        cen=(ed+gap+d/2 , 0)
        core_outer = self.add_round(
            layer=('si_etch', 'drawing'),
            resolution=self.grid.resolution,
            rout=d/2,
            rin=0,
            center=cen)
    ...
    def place_contact_electrodes(self):
        ...
        wire1_bottom = -4*offset_distance
...
if __name__ == '__main__':
    spec_file = 'Single_ring_halfrib/specs/meta_lens.yaml'
