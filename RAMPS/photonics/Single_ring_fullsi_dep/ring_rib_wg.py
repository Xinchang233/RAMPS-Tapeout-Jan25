...
class RingRibWg_sr(BPG.PhotonicTemplateBase):
    ...
    def get_params_info(cls) -> dict:
        return dict(
            ...
            n_contact_offset='c',
            layer='hjkh',
            si_spoke_offset_percentage='SI contact spokes width adjustment'
        )
    ...
    def draw_Spokes(self, doping_spoke_info):
        ...
        self.draw_contact_spokes_and_via_stack(
            r_via=n_via_radius-0.1+0.05,
            spoke_num=self.spoke_num,
            offset=self.n_contact_offset,
            bot_layer=self.bottom_metal_electrode_layer,
            top_layer=self.grid.tech_info.get_layer_name(max(n_electrode_layer_ind_list)),
            label=label_n,
        )
...
if __name__ == '__main__':
    spec_file = 'Single_ring_fullsi_dep/specs/ring_rib_wg_specs.yaml'
